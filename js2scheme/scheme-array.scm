...
(define (j2s-array-set! this::J2SAssig mode return ctx)
   ...
   (define (box r rhs::J2SExpr)
      (if (unboxed-type? (j2s-type rhs))
	  (with-access::J2SExpr rhs (loc type)
	     (let ((tmp (gensym)))
		`(let ((,tmp ,r))
		    ,(j2s-scheme (J2SCast 'any (J2SHopRef/type tmp type)) mode return ctx))))
	  r))
	 ...
	 (with-access::J2SAref obj (array alen)
		...
		(let ((a (gensym 'array))
		   ...
		   `(let ((,a ,(j2s-scheme obj mode return ctx))
			  ...
			  (,r ,(j2s-scheme rhs mode return ctx)))
		       (js-array-index-set! ,a ,f ,(box r rhs)
			  ,(strict-mode? mode)
			  %this)
		       ,r)))
	       ((int32)
		(let ((a (gensym 'array))
		   ...
		   `(let ((,a ,(j2s-scheme obj mode return ctx))
			  (,f ,(j2s-scheme field mode return ctx))
			  (,r ,(j2s-scheme rhs mode return ctx)))
		       (js-array-fixnum-set! ,a (int32->fixnum ,f) ,(box r rhs)
			  ,(strict-mode? mode)
			  %this)
		       ,r)))
	       ((fixnum int53)
		(let ((a (gensym 'array))
		   ...
		   `(let* ((,a ,(j2s-scheme obj mode return ctx))
			   (,f ,(j2s-scheme field mode return ctx))
			   (,r ,(j2s-scheme rhs mode return ctx)))
		       (js-array-fixnum-set! ,a ,f ,(box r rhs)
			  ,(strict-mode? mode)
			  %this)
		       ,r)))
	       (else
		   ...
		   ((string)
			...
			(with-access::J2SString field (val loc)
			   (if (string=? val "length")
				  ...
				  `(let* ((,a ,(j2s-scheme obj mode return ctx))
					  (,r ,(j2s-scheme rhs mode return ctx)))
				      (js-array-length-set! ,a ,(box r rhs)
					 ,(strict-mode? mode)
					 %this))
				  (error/location "hopc"
			...
			(let ((a (gensym 'array))
			   ...
			   `(let* ((,a ,(j2s-scheme obj mode return ctx))
				   (,f ,(j2s-scheme field mode return ctx))
				   (,r ,(j2s-scheme rhs mode return ctx)))
			       (js-array-string-set! ,a ,f ,(box r rhs)
				  ,(strict-mode? mode)
				  %this)
			       ,r))))
		   ((fixnum int53)
	    ...
	    (with-access::J2SExpr field (range)
		  ...
		  ((eq? (j2s-type field) 'uint32)
		   (let ((i (gensym 'idx)))
			  ...
			  ,(if (or (not vrange)
				   (not (eq? (interval-type vrange) 'integer)))
			       (let ((o (gensym 'obj)))
				  `(let ((,o ,(j2s-scheme obj mode return ctx)))
				      (if (<u32 ,i (vector-length ,o))
					  (vector-ref ,o (uint32->fixnum ,i))
					  (js-undefined))))
			       `(if (<u32 ,i ,(llong->uint32 (interval-max vrange)))
				    (vector-ref
				       ,(j2s-scheme obj mode return ctx)
				       (uint32->fixnum ,i))
				    (js-undefined))))))
		  ((or (not vrange) (not (eq? (interval-type vrange) 'integer)))
		   (let ((o (gensym 'obj))
			 (i (gensym 'idx)))
		      `(let ((,o ,(j2s-scheme obj mode return ctx))
			     (,i ,(j2s-scheme-as-integer field
				     mode return ctx)))
		  ...
		  ((and (interval? range)
			...
			(< (interval-max range) (llong->fixnum (interval-max vrange))))
		   `(vector-ref ,(j2s-scheme obj mode return ctx)
		       (uint32->fixum
			  ,(j2s-scheme-as-uint32 field mode return ctx))))
		  ((and (interval? range) (>=llong (interval-min range) #l0))
		   (let ((i (gensym 'idx)))
