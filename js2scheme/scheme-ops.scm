	 ...
	 ((eq? tl 'integer)
	  (if (eq? tr 'integer)
	      `(if (and (not (=fx ,right 0))
		   ...
		   (/flA ,(todouble left tl ctx) ,(todouble right tr ctx)))
	      `(/flB ,(todouble left tl ctx) ,(todouble right tr ctx))))
	 ((and (eq? tl 'int53) (m64? (context-conf ctx)))
	  (if (eq? tr 'integer)
	      `(if (and (not (=fx ,right 0))
			(=fx (remainderfx ,left ,right) 0))
		   (fixnum->flonum (/fx ,left ,right))
		   (/flC ,(todouble left tl ctx) ,(todouble right tr ctx)))
	      `(/flD ,(todouble left tl ctx) ,(todouble right tr ctx))))
