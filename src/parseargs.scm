...
;*    Last change :  Wed Jun 11 18:32:32 2014 (serrano)                */
...
;*---------------------------------------------------------------------*/
(define (hop-clientc-filename-resolver name this-or-path)
   (cond
      ((string-suffix? ".js" name)
       (nodejs-resolve name this-or-path))
      (else
       (find-file/path name this-or-path))))
