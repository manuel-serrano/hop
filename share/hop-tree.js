...
/*    Last change :  Mon Jul 20 11:33:28 2009 (serrano)                */
...
function hop_tree_close( tree ) {
   if( tree.openp ) {
	 ...
	 if( children[ i ].treeType === "hop-tree" ) {
...
function hop_tree_row_set_select_all( tree, select ) {
   ...
   var traverse_lr = function( tree ) {
      if( tree.treeType === "hop-tree-leaf" ) {
	 if( select ) {
...
/*    hop_make_tree_leaf ...                                           */
/*---------------------------------------------------------------------*/
function hop_make_tree_leaf( tree, klass, content, value, icon, iconerr ) {
