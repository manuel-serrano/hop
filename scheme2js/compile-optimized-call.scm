...
(define (infix-op nb-operands-min nb-operands-max infix-operator #!optional default-val)
   (lambda (p env compile operands)
	 ...
	 (and (>=fx nb-operands nb-operands-min)
	      ...
	      (begin
		 (if (=fx nb-operands 0)
		     (template-display p env "$default-val")
...
(define (minus-op p env compile operands)
   (cond
      ((null? operands) #f)
      ((null? (cdr operands))
       (template-display p env
	  ...
	  (compile (car operands) p #f))
       #t)
      (else
       ((infix-op 1 #f "-") p env compile operands))))

(define (div-op p env compile operands)
   (cond
      ((null? operands) #f)
      ((null? (cdr operands))
       (template-display p env
...
(define (vector-op p env compile operands)
   (template-display p env
      "[~e]"
      (separated ", "
...
(define (id p env compile operand)
   (and (pair? operand)
	(null? (cdr operand))
	(begin
...
(define (jsNew-op p env compile operands)
...
(define (jsCall-op p env compile operands)
   (and (not (null? operands))
...
(define (jsMethodCall-op p env compile operands)
   (and (not (null? operands))
	(not (null? (cdr operands)))
	(begin
	   (template-display p env
...
(define (symbolAppend_immutable-op p env compile operands)
   (let ((nb-operands (length operands)))
      (cond
	 ((= nb-operands 0)
	  (template-display p env "'\\uEBAC'")) ;; sc_SYMBOL_PREFIX
...
(define (stringAppend_mutable-op p env compile operands)
   ...
   (let ((nb-operands (length operands)))
      (cond
	 ((= nb-operands 0) (template-display p env "(new sc_String(''))"))
	 ((= nb-operands 1) (compile (car operands) p #f))
	 (else
	  (template-display p env
	     "(new sc_String(~e~e))"
...
(define (modulo-op p env compile operands)
   ;; TODO: get rid of config.
   (if (config 'correct-modulo)
       #f
       ((infix-op 2 2 "%") p env compile operands)))

(define (values-op p env compile operands)
   (let ((nb-operands (length operands)))
      (cond
	 ((= nb-operands 0) (template-display p env "(new sc_Values([]))"))
...
(define (not-op p env compile operands)
   (let ((nb-operands (length operands)))
      (and (= nb-operands 1)
...
(define (string2jsstring_mutable-op p env compile operands)
   (let ((nb-operands (length operands)))
      (cond
	 ((= nb-operands 0) (template-display p env "''"))
	 ((= nb-operands 1)
	  (let ((operand (car operands)))
	     (if (Const? operand)
		 (with-access::Const operand (value)
...
(define (symbol2jsstring_immutable-op p env compile operands)
   (let ((nb-operands (length operands)))
      (cond
	 ((= nb-operands 0) (template-display p env "''"))
	 ((= nb-operands 1)
