...
(define (optimize-tail-rec call-node target)
   ...
   (let* ((fun-body target.body)
      ...
      (new Begin (append formal-vaarg-assigs
			 (list (new Tail-rec-call tail-rec-label))))))
	  ...
	  (let* ((tail-rec-label this.tail-rec-label)
		 ...
		 (copy-assigs (if fun-vaarg
				  (cons (tail-rec-vaarg.var.assig
					...
					formals-copy)
				  formals-copy))
		 (tail-rec-body (new Begin (append copy-assigs
						   (list new-body))))
		 (loop (new Tail-rec tail-rec-formals
