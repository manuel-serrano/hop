...
;*    Last change :  Tue Mar 26 18:44:35 2024 (serrano)                */
	  ...
	  (lambda (ip)
		...
		((module ?- ??- (export . ?clauses) . ?-)
		 ...
		 (let ((env (filter-map parse-class clauses)))
		    (for-each (lambda (c) (print-class-node c env)) env)))))))
      (else
       (error "mkjsast" "Illegal command line" (format "~(, )" argv)))))
...
(define (print-class-node class env)
   ...
   (define (print-class-javascript name super fields)
      (printf "export function ~a(props) {\n" name)
      (printf "   this.$class = '~a';\n" name)
      (for-each (lambda (field)
		   (let ((d (cadddr field)))
		      (when (pair? d)
			 (printf "   this['~a'] = ~a;\n" (car field) (tojs (cadr d))))))
	 (cdr fields))
      (printf "   Object.assign(this, props);\n" (arg-name (car fields)))
      (printf "}\n")
      (when super
	 (printf "~a.prototype = new ~a();\n" name super))
      (printf "~a.prototype.duplicate = function(prop) {\n" name)
