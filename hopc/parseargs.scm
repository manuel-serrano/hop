...
(define (usage args-parse-usage)
   ...
   (print "      hopscript:pgo (alias of \"hopscript:cache hopscript:call format:pgo\")")
   (print "      format:json, format:pgo, format:memviz, srcfile=path, logfile=path")
...
(define (parse-args args)
   (let ((rc-file #unspecified)
	 ...
	 (args-parse (cdr args)
	    ...
	    (section "Profiling")
	    (("-fpgo" ?log (help "Profile log file optimization"))
	    ...
	    (("--profile-snapshot" (help "Profiling mode (see HOPTRACE)"))
	     (hopc-bigloo-profile-options-set! '("-srfi" "profile"))
	     (hopc-j2s-flags-set! (cons* :profile-snapshot #t (hopc-j2s-flags))))
