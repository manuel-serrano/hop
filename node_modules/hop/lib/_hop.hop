...
;*    Last change :  Thu May 16 09:08:29 2024 (serrano)                */
   ...
   (js-export (createScheduler
	 ...
	 (js-function (lambda (this proc path)
			 (unless scheduler-async
			    (set! scheduler-async
			       (worker-services-init %this worker)))
...
;*---------------------------------------------------------------------*/
;*    worker-services-init ...                                         */
;*---------------------------------------------------------------------*/
(define (worker-services-init %this worker)
   (with-access::JsGlobalObject %this (js-service-prototype)
      ;; extend the Service.prototype object with a schema method
      ;; used by service frames to retrieve their http schema
      (js-put! js-service-prototype (& "schema")
	 (js-function (lambda (this)
			 (when (isa? this JsService)
			    (with-access::JsService this (srv)
			       (let ((cfg (js-get srv (& "config") %this))
				     (ports (js-get srv (& "ports") %this)))
				  (tprint "cfg=" (typeof cfg) " p=" (typeof ports))
				  (when (and (js-jsobject? cfg)
					     (js-object? ports))
				     (let* ((n (js-get cfg (& "hostname") %this))
					    (s (js-get ports (& "https") %this))
					    (h (js-get ports (& "http") %this)))
					(tprint "n=" n)
					(if (integer? s)
					    (js-string->jsstring
					       (format "https://~a:~a"
						  (js-tostring n %this) s))
					    (js-string->jsstring
					       (format "https://~a:~a"
						  (js-tostring n %this) h)))))))))
	    "schema")
	 #f %this))
   (with-access::WorkerHopThread worker (%loop)
      ;; create a dummy async object to prevent the uv loop to terminate
      (instantiate::UvAsync
	 (loop %loop)
	 (cb list))))
...
;*    js-create-service ...                                            */
;*---------------------------------------------------------------------*/
(define (js-create-service %this::JsGlobalObject impl path loc register import worker::WorkerHopThread table srv)
