...
/*    Last change :  Thu Mar 28 08:20:29 2024 (serrano)                */
...
/*    loadUsersConfig ...                                              */
/*---------------------------------------------------------------------*/
function loadUsersConfig(descr) {
   if (descr.match(/\.json$/)) {
      config.users = JSON.parse(readFileSync(descr));
...
/*    init ...                                                         */
/*---------------------------------------------------------------------*/
function init(args = undefined, cfg) {
   if (!Object.isFrozen(config)) {
	 ...
	 if ("config" in args) {
      ...
      // users
      if ("users" in args) {
	 config.users = loadUsersConfig(users.args);
	 ...
	 if (existsSync(file)) {
	    config.users = loadUsersConfig(file);
	 } else {
	    config.users = [];
	    console.error("warning, no users database configured!");
	 }
      }
...
function parseCommandLine(argv) {
   ...
   function help() {
      ...
      console.log("  --users FILE           Set user database name");
      console.log("");
