...
/*    Last change :  Thu Jan 18 15:42:06 2024 (serrano)                */
...
/*    HipHop extension                                                 */
/*---------------------------------------------------------------------*/
const extensionsRegex = /\.(hop\.m?js)$/;
...
export async function resolve(url, context, nextResolve) {
   if (extensionsRegex.test(url)) {
      ...
      const transformedSource = await HopCompile(new URL(nurl));
...
export async function load(url, context, nextLoad) {
   if (extensionsRegex.test(url)) {
      return nextLoad("file://" + await HopCompile(url));
   } else {
      return nextLoad(url);
   }
}
...
/*    HopCompile ...                                                   */
/*---------------------------------------------------------------------*/
async function HopCompile(url) {
   const path = fileURLToPath(url);
   const target = dirname(path) + "/._" + basename(path).replace(extensionsRegex, ".hopc.mjs");
