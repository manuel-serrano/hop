...
/*    Last change :  Fri May  3 07:44:54 2024 (serrano)                */
...
class app {
   ...
   listen() {
	 ...
	 const hdl = this.#handlerTable.get(url)[req.method];
	 ...
	 if (hdl) {
	    ...
	    throw new Error(`hop; no ${method} handler for URL ${url}`);
	 }
      }
	 ...
	 if (this.#hop.config.ports.http >= 0) {
	    bcastServer = httpServer = http.createServer(handler)
	 ...
	 if (this.#hop.config.ports.https >= 0) {
	    ...
	    bcastServer = httpsServer = https.createServer(credentials, handler)
