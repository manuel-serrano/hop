...
/*    Last change :  Sun Jan 28 06:25:17 2024 (serrano)                */
...
export class Resolver {
   ...
   resolve(mod) {
	 ...
	 const nextr = async (s, c) => {
	    console.error("resolver.next s=", s, "index=", index, hooks.length, "root=", root);
	 ...
	 return loader.resolve(mod, ctx, nextr)
	    .then(u => {
	       ...
	       let root = resolveRootTable.get(dir);

	       if (!root) {
		  root = hopBase + "/$/" + resolveRootTable.size + "/" + dir;
		  app.use(root, express.static(dir));
		  resolveRootTable.set(dir, root);
	       }

	       const url = path.resolve(root + "/" + mod);
