...
/*    Last change :  Thu Jul 11 17:09:56 2024 (serrano)                */
...
class ServiceImpl {
   ...
   static postResponse(req, res) {
	 ...
	 if (jsargs && jsargs.startsWith("hop-encoding=hopjs&vals=")) {
	    rep = this.#fun.apply(req,decode(req, jsargs));
	 } else {
...
function decode(req, query) {
   ...
   const initrx = /hop-encoding=hopjs&vals=([0-9]+)[.]/y;
   const lenrx = /([0-9]+)[.]/y;
   ...
   const m = query.match(initrx);

   if (m) {
      const len = parseInt(m[1]);
      return decodeArgs(req, query, new Array(len), len, initrx.lastIndex);
   } else {
      err(req);
   }
}
