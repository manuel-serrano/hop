...
/*    Last change :  Tue Apr  2 07:57:53 2024 (serrano)                */
...
import { init as broadcastInit } from "./broadcast.mjs";
...
export { config, init, listen,
	 ...
	 HTTPResponseHTML };
export { broadcast } from "./broadcast.mjs";
...
function backendInit(config) {
   if (!backend) {
      ...
      backend = require(mod).init(config);
   }
}
...
function init(cfg = {}) {
   if (!config) {
      ...
      // configure the server-event (for broadcast)
      broadcastInit(config, backend.websockets, ServiceImpl);
