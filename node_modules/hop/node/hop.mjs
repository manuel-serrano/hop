...
/*    Last change :  Mon Apr 29 07:50:11 2024 (serrano)                */
...
export { Hop,
/* 	 config, init, listen,                                         */
/* 	 Resolver, register, resolve, Service, ServiceImpl, Frame,     */
...
class Hop {
   ...
   constructor(config, rcdir = undefined) {
      this.config = configInit(config, rcdir);

      // backend initialization
      const mod = (config.engine === "hop"
	 ? "./hop-backend.hop"
	 : "./node-backend.js");
      this.#backend = require(mod).init(this, registerWebsocket);

      // user authentication initialization
      this.auth = new UserAuth(this);

      // server broadcast initialization
      this.#broadcast = eventInit(this);
      new ServiceImpl(this, serverEventInit, "server/event/init");
   }
...
class Resolver {
   ...
   constructor(root, ...files) {
      ...
      files.forEach(m => this.register(m, root));
   ...
   url(path) {
      return this.#resolveTable.get(path);
   }
}
...
function register(path) {
   if (!hopResolver) {
	 ...
	 .resolve("@hop/hop/client.mjs");
      hopResolver = new Resolver(dirname(self));
   }
...
class Frame {
   ...
   toString() {
      ...
      return `http://${H.hostname}:${config.server.http}${this.#svc.path(this.#args)}`;
   }
}
...
class ResponseAuthentication extends ResponseString {
   hopResponse(req, res) {
      // authentication
      console.error("hop.mjs port=", req.port);
