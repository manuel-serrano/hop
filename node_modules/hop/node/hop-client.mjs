...
/*    Last change :  Mon Apr  1 07:48:35 2024 (serrano)                */
...
class Frame {
   ...
   static XMLHttpRequest(res, rej) {
	 ...
	 if (this.readyState === 4) {
	    switch (this.status) {
	       case 200:
		  res(decode(req,response, req.getResponseHeader("Hop-Serialize")));
		  break;
...
class ClientServer {
   ...
   addEventListener(event, proc, capture) {
      const evtHandler = {event, proc, capture};
      this.initWebSocket()
	 .then(_ => this.register(event))
	 .then(_ => this.#events = this.#events ? this.#events.push(evtHandler) : [proc]);
   }

   initWebSocket() {
      new Promsie((res, rej) => {
	 new Frame("/hop/server-event/info", [], [], [])
	    .post()
	    .then(val => {
	       if (!this.#ws) {
		  const url = `${val.ssl ? "wss" : "ws"}//${host}:${port}/hop/server-event?key=${val.key}`;
		  this.#key = val.key;
		  this.#host = val.host;
		  this.#port = val.port;
		  
		  this.#ws = new WebSocket(url);
		  this.#ws.addEventListener("message", e=> this.rcvMessage(e));
		  this.#ws.addEventListener("open", e => {
		     setInterval(() => { /* TODO ping */ }, pingTime * 1000);
		     res(this.#ws);
		  });
	       } else {
   ...
   register(event) {
	 ...
	 case 1: {
	    // open
	    return new Frame(`/hop/server-event/register?event=${encodeURIComponent(event)}&key=${this.#key}`)
	       .post()
	 }
   ...
   rcvMessage(event) {
      const m = event.data.match(/^([a-z]+):([a-zA-Z0-9%]_):(.*)$/m);
      if (m) {
	 const val = decode(m[3], m[1]);
	 const id = decodeURIComponent(m[2]);
	 const hdls = this.#events(id);

	 if (hdls) {
	    const evt = new HopServerEvent(id, val);
	    for (let h of hdls) {
	       h.proc(evt);
