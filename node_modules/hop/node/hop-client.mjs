...
/*    Last change :  Tue Apr  2 07:37:17 2024 (serrano)                */
...
class ClientServer {
   ...
   addEventListener(event, proc, capture) {
	 ...
	 .then(_ => this.#events[event] = evtHandler);
   }

   connect() {
      return new Promise((res, rej) => {
	 new Frame(undefined, "/hop/server/event-info", [this.pingTime], [])
	    .post()
	    .then(val => {
	       console.log("CONNECT VAL=", val, this.#ws);
	       if (!this.#ws) {
		  ...
		  this.#ws.addEventListener("open", e => {
		     console.log("ws open...");
		     setInterval(() => this.ping(), this.pingTime * 1000);
		     res(this.#ws);
		  });
	       } else {
		  console.log("CONNECT VAL=", val, this.#ws, this.#ws.readyState);
   ...
   register(event) {
      console.log("register event=", event, this.#ws.readyState);
