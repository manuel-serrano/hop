...
/*    Last change :  Wed Jul 10 15:44:11 2024 (serrano)                */
...
export class XmlElement {
   ...
   constructor(tag, attrs, nodes) {
      this.tag = tag;
      this.attrs = XmlElement.attrsCheck(tag, attrs);
      this.nodes = nodes;
   }

   static attrsCheck(tag, attrs) {
      if (attrs === undefined) {
	 return attrs;
      } else if (typeof attrs === "object") {
	 Object.keys(attrs).forEach(k => {
	    const t = typeof attrs[k];
	    if (t !== "string" && t !== "number" && t !== "boolean" && t !== "object") {
	       const sattrs = Object.keys(attrs).map(k => `${k}="${attrs[k]}"`);
	       const obj = `<${tag} ${sattrs.join(" ")}>...</${tag}>`;
	       throw new Error(`Illegal Xml element attribute "${obj}"`);
	    }
	 });
	 return attrs;
      } else {
	 const obj = `<${tag} ${sattrs}>...</${tag}>`;
	 throw new Error(`Illegal Xml element attribute "${obj}"`);
      }
   }
   ...
   toResponse() {
      return { string: this.toString(), serialize: "html" };
   }
...
/*---------------------------------------------------------------------*/
/* @sealed */
export class Tilde extends XmlElement {
   toString() {
      if (!this.tostring) {
...
export function AUDIO(attrs, ...nodes) {
   return new XmlElement("audio", attrs, nodes);
}
...
export function VIDEO(attrs, ...nodes) {
   return new XmlElement("video", attrs, nodes);
}
...
export function DOLLAR(expr, loc) {
   ...
   switch (typeof expr) {
      case "string": return "'" + purify(expr) + "'";
