...
/*    Last change :  Wed May  1 09:08:58 2024 (serrano)                */
...
export { UserAuth, digestPassword, WWWAuthenticate };

/*---------------------------------------------------------------------*/
/*    digestPassword ...                                               */
/*---------------------------------------------------------------------*/
function digestPassword(name, pass, realm) {
...
/*    getDigestOpaque ...                                              */
/*---------------------------------------------------------------------*/
function getDigestOpaque(req, port) {
   if (!digestOpaque) {
      digestOpaque = `${req.hop.hostname}:${req.port}:${digestSession}`;
   }
   return digestOpaque;
}
...
function authenticationDigest(hop, req, digest, user) {
   ...
   if (digest.opaque === getDigestOpaque(req)
...
class UserAuth {
   ...
   #usersMap = new Map();
   #userAnonymous = { name: "anonymous", services: [], directories: [] };
   ...
   authorizedService(req, url) {
	 ...
	 if (auth) {
	    ...
	    if (match = auth.match(/^Digest /)) {
	       ...
	       const user = this.#usersMap.get(digest.username);
	       const authuser = authenticationDigest(this.#hop, req, digest, user)
		  ? user
		  : this.#userAnonymous;
	       
	       return authuser?.services === "*"
		  || authuser?.services.indexOf(url) >= 0
		  || authuser?.services.find(s => {
		     if (url[s.length] === "/") {
