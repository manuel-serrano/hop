/*=====================================================================*/
/*    /tmp/TBR/hhmqtt/node_modules/@hop/hopc/node/generate.mjs         */
/*    -------------------------------------------------------------    */
/*    Author      :  Manuel Serrano                                    */
/*    Creation    :  Thu Nov  2 14:38:40 2023                          */
/*    Last change :  Thu Jul 11 12:34:12 2024 (serrano)                */
...
const generate = (prog, conf = {}) => {
   return prog.generate(new Conf(conf.context || "server", null));
}
...
J2SDecl.prototype.generate = function(conf) {
   ...
   if (conf.typescript && this.utype) {
      ...
      if (ty !== "unknown") {
	 return new Fragment(conf, this, binder, this.id, opt, ":", ty, sep);
      }
   }
   return new Fragment(conf, this, binder, this.id, sep);
}
...
J2SDeclInit.prototype.generate = function(conf) {
   ...
   if (conf.typescript && this.utype) {
	 ...
	 return new Fragment(conf, 
	    loc, binder,
			     new Fragment(conf, loc, this.id, opt, ":", ty,
   ...
   return new Fragment(conf, 
      this, binder,
		       new Fragment(conf, loc + binder.length, this.id, "=", this.val.generate(conf), sep));
}
...
J2SAccess.prototype.generate = function(conf) {
   ...
   if (m) {
      return new Fragment(conf, 
	 this, obj,
			  new Fragment(conf, this.field, ".", m[1]));
   } else {
      return new Fragment(conf, 
	 this, obj,
			  new Fragment(conf, this.field, "[", field, "]"));
   }
}
...
J2STilde.prototype.generate = function(conf) {
   const nconf = new Conf("client", conf);
