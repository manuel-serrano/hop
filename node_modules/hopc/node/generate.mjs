/*=====================================================================*/
/*    .../node_modules/@hop/hopc/lib/generate.mjs                      */
/*    -------------------------------------------------------------    */
/*    Author      :  Manuel Serrano                                    */
/*    Creation    :  Thu Nov  2 14:38:40 2023                          */
/*    Last change :  Thu Feb 15 14:03:52 2024 (serrano)                */
...
import { J2SNode, J2SProgram, J2SBlock, J2SLetBlock,
	 ...
	 J2SIf, J2SSwitch, J2SCase, J2SDefault, J2SCatch, J2STry, J2SThrow,
	 ...
	 J2SDollar, J2STilde } from "./ast.mjs";
...
class Fragment {
   ...
   mappings(source) {
	 ...
	 if (!line) {
	    console.error("SOURCEMAP ERROR, cannot find location", loc, "source=", source);
...
J2SDecl.prototype.generate = function(conf) {
   if (conf.typescript && this.utype) {
      const ty = typeof this.utype === "symbol"
	 ? this.utype.toString().match(/\(([^)]+)\)/)[1] : this.utype;
      if (ty !== "unknown") {
	 return new Fragment(this, this.id, ":", ty);
      }
   }
   return new Fragment(this, this.id);
}
...
J2SDeclInit.prototype.generate = function(conf) {
   ...
   if (conf.typescript && this.utype) {
	 ...
	 return new Fragment(
	    this, binder,
	    new Fragment(loc + binder.length, this.id, ":", ty,
   ...
   return new Fragment(
      this, binder,
      new Fragment(loc + binder.length, this.id, "=", this.val.generate(conf), ";"));
}
...
J2SDeclFun.prototype.generate = function(conf) {
   return new Fragment(this, `let ${this.id} = `, this.val.generate(conf), this.expression ? "" : ";");
}
...
J2SDeclSvc.prototype.generate = function(conf) {
   return new Fragment(this, "hop.service(", this.val.generate(conf),
...
J2SArrow.prototype.generate = function(conf) {
   ...
   if (conf.typescript && this.rutype) {
      const ty = typeof this.rutype === "symbol"
	 ? this.rutype.toString().match(/\(([^)]+)\)/)[1] : this.rutype;
      if (ty !== "unknown") {
...
J2STemplate.prototype.generate = function(conf) {
   const nodes = list2array(this.exprs).map(n => {
	 ...
	 return new Fragment(this, "${", n.generate(conf), "}");
      }
   });
...
J2SDollar.prototype.generate = function(conf) {
   if (this.context === "tilde-expression" || this.context === "script-expression") {
      return new Fragment(this, "${DOLLAR(", this.node.generate(conf), ")}");
   } else {
...
J2STilde.prototype.generate = function(conf) {
   return new Fragment(this, "TILDE(undefined, `", this.stmt.generate(conf).toString(), "`)");
}
