...
/*    Last change :  Wed Nov 12 08:16:13 2025 (serrano)                */
...
import { J2SNode, J2SProgram, J2SBlock, J2SLetBlock,
	 ...
	 J2SFun, J2SArrow, J2SSvc, J2SDeclFun, J2SDeclSvc, J2SReturn, J2SYield, 
...
class Fragment {
   ...
   toString() {
	 ...
	 return this.val.map(v => {
	    ...
	    console.error("ERROR V=", this.val);
	 }
      } else {
	 // client mode
	 return this.val.map(v => {
...
class FragmentList {
   ...
   toString() {
	 ...
	 if (this.val.length === 0) {
	    ...
	    }).filter(n => n).join(`+${stringify(this.sep)}+`);
...
J2SDeclSvc.prototype.generate = function(conf) {
   return new Fragment(conf, this, "const ", this.id, " = ",
		       this.val.generate(conf), this.expression ? "" : ";");
}
/* J2SDeclSvc.prototype.generate = function(conf) {                    */
...
J2SArrow.prototype.generate = function(conf) {
   ...
   if (conf.typescript && this.rutype) {
      ...
      if (ty !== "unknown") {
	 return new Fragment(conf, this, opar, args, "): ", ty, " => ", body.generate(conf));
      }
   }

   return new Fragment(conf, this, opar, args, ") => ", body.generate(conf));
}

/*---------------------------------------------------------------------*/
/*    generate ::J2SSvc ...                                            */
/*---------------------------------------------------------------------*/
J2SSvc.prototype.generate = function(conf) {
   const fun = generateFun.call(this, conf, "", "function ");
   const name = this.path ? stringify(this.path) : "undefined";
   return new Fragment(conf, this, "Service(", fun, ", ", name, ")");
}
...
J2STry.prototype.generate = function(conf) {
   const final = this.finally.$class === "J2SNop" ? "" : new Fragment(conf, this, "finally { ", this.finally.generate(conf), "}\n");
   return new Fragment(conf, this, "try ", this.body.generate(conf), this.catch.$class === "J2SNop" ? "" : this.catch.generate(conf), final);
}
...
J2SClass.prototype.generate = function(conf) {
   return new Fragment(conf, 
      this, "class ",
      this.name ? this.name : "",
      !this.super instanceof J2SUndefined ? new Fragment(conf, this.super, " extends ",
      this.super.generate(conf)) : "",
      " {", "\n",
...
J2STilde.prototype.generate = function(conf) {
   ...
   const node = this.stmt.generate(nconf);
   return new Fragment(conf, this, "TILDE(undefined, ", node, ")");
}
