...
/*    Last change :  Fri Nov 24 17:28:43 2023 (serrano)                */
...
class Fragment {
   ...
   mappings(source) {
	 ...
	 if (typeof e === "string") {
	    ...
	    if (e.offset) dstlines[line][col] = e.offset;
	    //console.log("--> [" + line + "][" +col + "]=", e.offset);
	    for (let f of e.val) {
	       walk(f);
	    }
	 ...
	 return { oline: line.num, ocol: loc - line.start };
      ...
      let sourcemapCol = 0, sourcemapOline = 0, sourcemapOcol = 0;
	 ...
	 for (let k in line) {
	    if (!line[k]) console.error("SOURCEMAP ERROR, wrong line", line);
	    const { oline: ol, ocol: oc } = findOriginalLocation(line[k]);
	    //console.error(k, "(" + line[k] + ")", "->", ol, oc);
	    components.push(vlq(k - sourcemapCol) + "A" + vlq(ol - sourcemapOline) + vlq(oc - sourcemapOcol));
	    sourcemapCol = k;
	    sourcemapOline = ol;
	    sourcemapOcol = oc;
	 }
