...
/*    Last change :  Fri May 16 08:50:42 2025 (serrano)                */
...
class XMLLexer extends Lexer {
   ...
   match(port, val, rule) {
	 ...
	 case "HTML": {
	    const sym = identUpcase(val.substring(0, val.length - 1));
	 ...
	 case "OHTML": {
	    const str = identUpcase(val);
	    port.offset += val.length;
	    return this.openTag("OHTML", str, null, port);
	 }
	    
	 case "CHTML": {
	    const sym = "<" + identUpcase(val.substring(2, val.length));
	    port.offset += val.length;
	 ...
	 case "OTAG": {
	    const t = identUpcase(val.substring(0, val.length - 1));
	    const sym = t + ">";
	    let attr = null;
	    port.offset += val.length;
