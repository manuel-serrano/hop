...
/*    Last change :  Fri Jan 19 19:08:51 2024 (serrano)                */
...
import { J2SNode, J2SString, J2SUndefined, J2SCall, J2SDataPropertyInit, J2SObjInit } from "./ast.mjs";
...
class XMLLexer extends Lexer {
   ...
   match(port, val, rule) {
	 ...
	 case "STRING": {
	    ...
	    const res = this.pushIgnore(new J2SString(props));
	    return { res, ignore: false };
	 }
	 case "OHTML": {
	    const str = val.toUpperCase();
	    const res = this.openTag("OHTML", str, null, port);
	    return { res, ignore: false };
	 }
	 case "CHTML": {
	 ...
	 case "OTAG": {
	    ...
	    while (true) {
		  ...
		  const tag = new Token("HTML", sym, makeLoc(port));
		  this.pushNodeIgnore(makeDomCreate(tag, reverse$(attr), this.#lang, this.#conf));
	       } else if (obj !== ">") {
		  return xmlParseError("illegal character", obj, inputPortName(port), inputPortPosition(port));
	       } else {
		  const res = this.openTag("HTML", sym, reverse$(attr), port);
		  port.offset -= val.length;
		  return { res, ignore: false };
	       }
	    }
...
class AttrLexer extends Lexer {
   ...
   match(port, val, rule) {
	 ...
	 case "J": {
	    const loc = makeLoc(port);
	    const key = val.replace(/[ \t]*=/, "");
	    port.offset += val.length;
	    const str = read_rp(new AttrValLexer(), port);
	    const props = {
	       escape: ["escape"],
	       val: key,
	       loc: makeLoc(port)
...
/*    htmlParseScript ...                                              */
/*---------------------------------------------------------------------*/
function htmlParseScript(port, conf, plugins) {
   ...
   console.error("SCRIPT=", port.buffer.substring(0, 50));
