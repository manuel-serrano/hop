...
/*    Last change :  Tue Jan 23 05:27:02 2024 (serrano)                */
...
class J2SLexer extends Lexer {
   ...
   match(port, val, rule, ...opts) {
	 ...
	 case "TEMPLATE": {
	    port.offset++;
	    return { res: templateParser(port, 0), ignore: false };
	 }
	 case "OHTML": {
	    const i = val.match(/[ \t\n]/).index;
...
templateLexer.reader = port => templateParser(port, 1);
...
/*    templateParser ...                                               */
/*---------------------------------------------------------------------*/
function templateParser(port, backquoteShift) {
   const buffer = port.buffer;
   let offset = port.offset;
   ...
   while (true) {
	 ...
	 const loc = makeLoc(port);
	 port.offset = idxBackquote + backquoteShift + 1;
