...
/*    Last change :  Fri Jan 19 18:36:41 2024 (serrano)                */
...
class J2SLexer extends Lexer {
   ...
   match(port, val, rule, ...opts) {
	 ...
	 case "TEMPLATE": {
	    port.offset++;
	    return { res: templateParser(port, 0, 0), ignore: false };
	 }
	 case "OHTML": {
	    const i = val.match(/[ \t\n]/).index;
...
templateLexer.reader = port => templateParser(port, 1, 1);
...
/*    templateParser ...                                               */
/*---------------------------------------------------------------------*/
function templateParser(port, backquoteShift, dollarShift) {
   const buffer = port.buffer;
   let offset = port.offset;
   ...
   while (true) {
	 ...
	 const loc = makeLoc(port);
	 port.offset = idxBackquote + backquoteShift;
	 ...
	 if (buffer[idxDollar + 1] === "{") {
	    ...
	    const loc = makeLoc(port);
	    port.offset = idxDollar + dollarShift;
