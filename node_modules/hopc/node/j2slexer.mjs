...
/*    Last change :  Fri Dec  1 20:55:15 2023 (serrano)                */
...
function templateParser(port) {
   ...
   let id = buffer.indexOf("$", port.offset);
   let ib = buffer.indexOf("`", port.offset);

   TODO....
      
   if (ib < id) {
      // we are done parsing the string
      const val = buffer.substring(buffer.offset + 1, ib);
      port.offset = ib;
      ...
      if (buffer[id + 1] === "{") {
	 // a true escape
	 return new Token("TEMPLATE", val);
      } else {
	 prefix = buffer.substring(buffer.offset + 1, ib);
	 port.offset += id;
      }
      let id = buffer.indexOf("$", port.offset);
      let ib = buffer.indexOf("`", port.offset);
