...
/*    Last change :  Tue Jan 23 08:12:13 2024 (serrano)                */
...
class J2SLexer extends Lexer {
   eof(port, ...opts) {
      let i = 1;
      while (port.offset - i > 0 && port.buffer[port.offset - i].match(/[\n \t]/)) {
	 // on eof find the first non blank character
	 i++;
      }
      port.offset-=i;
      let tok = new Token("EOF", "eof", makeLoc(port));
      port.offset+=i;
      return tok;
   ...
   match(port, val, rule, ...opts) {
	 ...
	 case "CTAG": {
	    const res = new Token(rule.type, "<" + val.substring(2, val.length-1) + ">", makeLoc(port));
	    port.offset += val.length;
	    return { res, ignore: false };
	 }
	 case "__SELF__": {
...
function rxParser(port) {
   ...
   while (true) {
	 ...
	 return new Token("RegExp", cons(val, flags), loc);
      } else if (escoffset >= 0 && escoffset < rangeoffset) {
	 offset = escoffset + 1;
      } else {
...
function templateParser(port) {
   ...
   // console.error(">>> templateParser:", buffer.substring(offset, offset + 30));
   while (true) {
	 ...
	 //console.error("<<< templateParser.1:", buffer.substring(port.offset, port.offset + 30));
	 ...
	 if (buffer[idxDollar + 1] === "{") {
	    ...
	    //console.error("<<< templateParser.2:", buffer.substring(port.offset, port.offset + 30));
...
/*    j2sEscapeJsString ...                                            */
/*---------------------------------------------------------------------*/
export function j2sEscapeJsString(s) {
   return s;
}
