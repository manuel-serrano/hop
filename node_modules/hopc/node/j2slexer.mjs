...
/*    Last change :  Tue Dec  5 16:27:58 2023 (serrano)                */
...
/*    templateParser ...                                               */
/*---------------------------------------------------------------------*/
function templateParser(port, dollarShift) {
   const buffer = port.buffer;
   let offset = port.offset;
   ...
   while (true) {
	 ...
	 return new Token("TSTRING", val.replace(/\r\n?/g, "\n"), loc);
      } else {
	 ...
	 if (buffer[idxDollar + 1] === "{") {
	    ...
	    return new Token("TEMPLATE", val.replace(/\r\n?/g), loc);
	 } else {
	    offset = idxDollar + 1;
	 }
      }
