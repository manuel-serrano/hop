/*=====================================================================*/
/*    .../prgm/project/hop/3.7.x/node_modules/hopc/node/reader.mjs     */
/*    -------------------------------------------------------------    */
/*    Author      :  manuel serrano                                    */
/*    Creation    :  Sat Sep 16 12:17:54 2023                          */
/*    Last change :  Fri Jan 19 17:47:09 2024 (serrano)                */
...
/*    makeLoc ...                                                      */
/*---------------------------------------------------------------------*/
export function makeLoc(port) {
   return new Location(port.filename, port.offset);
}
...
export class Lexer {
   ...
   error(port) {
      throw new Error(`${this.constructor.name} Unexpected input "${port.buffer.substring(0, 20)}..."`);
   }
   ...
   reader(port, ...opts) {
	 ...
	 if (buffer.length <= port.offset) {
	    ...
	    if (lix === -1) {
	       console.error("hopc:reader.mjs:NO MATCH@" + port.offset, "[" +
		  buffer.substring(port.offset, port.offset + 20) + "]");
               return this.error(port, ...opts);
	    } else {
	       const val = m[lix];
