...
;*    Last change :  Tue Jul 17 16:31:34 2018 (serrano)                */
	 ...
	 (let ((obj (js-new0 %this js-object)))
	    (js-bind! %this obj 'addPlugin
	       :value (js-make-function %this
			 (lambda (this key parser)
			    (set! plugins
			       (cons
				  (cons (string->symbol (js-tostring key %this))
				     (lambda (tok ctrl)
					(set! controller ctrl)
					(js->ast %this
	    ...
	    (js-bind! %this obj 'peekToken
	       :value (js-make-function %this
			 (lambda (this token)
				...
				(token->js ((vector-ref controller 0)))
	    ...
	    (js-bind! %this obj 'consumeAny
	       :value (js-make-function %this
			 (lambda (this token)
			    (if (vector? controller)
				(token->js ((vector-ref controller 2)))
				(js-raise-type-error %this
				   "detached plugin parser" this)))
			 1 'consumeAny)
	       :writable #f
	    ...
	    (js-bind! %this obj 'parseExpression
	       :value (js-make-function %this
			 (lambda (this token)
			    (if (vector? controller)
				(ast->js ((vector-ref controller 3) #f #f))
				(js-raise-type-error %this
				   "detached plugin parser" this)))
			 1 'parseExpression)
	       :writable #f
   ...
   (define parser
	 ...
	 (let ((obj (js-new0 %this js-object)))
	    (with-access::JsObject obj (__proto__)
	       (set! __proto__ parser-prototype)
	       obj))))
...
;* 		     (class-field-type f))                             */
		  (cond
			 ...
			 ((class-field-mutator f)
			  inst
			  (if (eq? (class-field-type f) 'symbol)
			     (if (string? v)
				 (string->symbol v)
				 (js-raise-type-error %this 
				    "js->ast: bad field type ~a, string expected"
				    (format "~a.~a" cname n)))
			     v))))
   ...
   (define (js->node n)
      (cond
	 ((isa? n J2SNode) n)
	 ((isa? n JsString) (js-jsstring->string n))
	 ((isa? n JsArray) (map! js->node (jsarray->list n %this)))
	 ((isa? n JsObject) (jsobject->node n))
	 (else n)))
