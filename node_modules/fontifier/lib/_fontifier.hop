;*=====================================================================*/
;*    /tmp/HOPNEW/hop/node_modules/fontifier/lib/_fontifier.hop        */
;*    -------------------------------------------------------------    */
;*    Author      :  Manuel Serrano, Florian Loitsch                   */
;*    Creation    :  Thu Sep  7 17:29:41 2006                          */
;*    Last change :  Sun Feb 23 15:47:52 2020 (serrano)                */
...
(define (hopscript %this this scope module)
   (&with!
	 ...
	 (fontifier-bind-funs! %this exports
	    xml hop hss ebnf plain c js hopscript tex sh hiphop asm)
   ...
   (define *asm-grammar*
	 ...
	 (else
	  (let ((c (the-failure)))
	     (if (eof-object? c)
		 (reverse! res)
		 (error "asm-fontifier" "Unexpected character" c))))))
...
(define *js-lexer*
   (regular-grammar ((blank (in #\Space #a012 #a013))
	  ...
	  (multiple-value-bind (id class)
	     ...
	     (let ((cur res))
		(set! res '())
		(let ((anchor (<SPAN> :data-charpos charpos
				 :id id :class class (ignore))))
		   (set! res cur)
		   (add! anchor)
		   (ignore))))))
      ((: "#/" (+ (out #\#)) #\#)
	...
	(: (or (: (+ digit) #\. (* digit)) (: #\. (+ digit)))
	  ...
	  (cond
	     ...
	     ((and (eq? symbol 'function)
		   (memq lang '(hopscript hiphop javascript)))
	      (add! (<SPAN> :data-charpos charpos
		       :class (string-append "fontifier-prog-define symbol-str")
		       symbol-str))
	      (ignore))
	     ...
	     ((and (getprop symbol 'hiphop) (eq? lang 'hiphop))
	      ...
	      (ignore))
	     ((and (eq? lang 'hiphop)
		   (memq symbol '(val preval now pre count)))
	      (add! (<SPAN> :data-charpos charpos
