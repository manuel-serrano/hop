...
;*    Last change :  Fri Dec 19 10:41:32 2014 (serrano)                */
...
(module _fontifier
   ...
   (export (hopscript ::JsGlobalObject ::JsObject ::JsObject ::JsObject)
	   ...
	   (hop-fontifier::pair-nil ::input-port
	      #!optional line-begin line-end keyword-fontifier)
	   ...
	   (c-fontifier::pair-nil ::input-port #!key line-begin line-end)
	   (js-fontifier::pair-nil ::input-port #!key line-begin line-end)
	   (hopscript-fontifier::pair-nil ::input-port #!key line-begin line-end)
...
(define-macro (fontifier-bind-fun! %this obj lang)
   (let ((jsfun (symbol-append lang 'Fontifier))
...
(define (hopscript %this this scope module)
   (let ((exports (js-get module 'exports %this)))
      (fontifier-bind-funs! %this exports xml hop hss ebnf plain c js hopscript tex)
...
(define *xml-grammar*
   (regular-grammar ()
	  ...
	  (cons (<SPAN> :class "fontifier-prog-line-comment" str)
		(ignore))))
      ((+ (or #\Newline #\Space))
       ;; separators
       (let ((str (xml-string-encode (the-string))))
	  (cons (<SPAN> str) (ignore))))
      ((: "#" (+ (in #\_ #\@ ("azAZ09"))) #\#)
       ;; anchor
       (multiple-value-bind (id class)
	  (fontifier-parse-ident (the-substring 1 -1))
	  (let ((body (ignore)))
	     (cons (<SPAN> :id id :class class body) (ignore)))))
      ((: "#/" (+ (in #\_ #\@ ("azAZ09"))) #\#)
       ;; end anchor
       '())
      ((: #\<
	  ...
	  (? #\>))
       ;; tag
       (let ((str (xml-string-encode (the-string))))
	  (cons (<SPAN> :class "fontifier-prog-tag" str)
		(ignore))))
      (#\>
       (cons (<SPAN> :class "fontifier-prog-tag" "&gt;")
	     (ignore)))
      ((: "<?" (: (? (in "/?")) alpha (* (out #\> #\space #\tab #\Newline))))
       ;; special
       (let ((str (xml-string-encode (the-string))))
	  (cons (<SPAN> :class "fontifier-prog-special" str)
		(ignore))))
      ("?>"
       ;; special
       (cons (<SPAN> :class "fontifier-prog-special" "?&gt;")
	     (ignore)))
      (#\<
       (cons "&lt;" (ignore)))
      ...
      ((or (: "\"" (* (or (out #a000 #\\ #\") (: #\\ all))) "\"")
       ...
       ;; strings
       (let ((str (xml-string-encode (the-string))))
	  (cons (<SPAN> :class "fontifier-prog-string" str)
		(ignore))))
      ((in "\"=#'")
       (let ((str (the-string)))
	  (cons str (ignore))))
...
;*    hop-fontifier ...                                                */
;*---------------------------------------------------------------------*/
(define (hop-fontifier ip #!optional line-begin line-end keyword-fontifier)
...
;*    hiphop-fontifier ...                                             */
;*---------------------------------------------------------------------*/
(define (hiphop-fontifier ip #!optional proc)
   (hop-fontifier ip
      (lambda (x)
	 (cond
	    ((string-suffix? "&amp;" x)
...
(define *js-lexer*
   (regular-grammar ((blank (in #\Space #a012 #a013))
	  ...
	  (add! (<SPAN> :class "fontifier-prog-tag" cpp))
	  (ignore)))
	...
	(: (or (: (+ digit) #\. (* digit)) (: #\. (+ digit)))
	  ...
	  (if (eq? lang 'hopscript)
	      (add! (<SPAN> :class "fontifier-prog-tag" tmp))
	      (add! tmp))
	  (ignore)))
	  ...
	  (set! column (+fx (the-length) column))
	  (add! (<SPAN> :class "fontifier-prog-keyword" kwd))
	  (ignore)))
...
(define (js-fontifier ip #!key line-begin line-end)
   (js-lexer-init!)
   (read/rp *js-lexer* ip 'javascript line-begin line-end (not line-begin) '() 0))
...
(define (hopscript-fontifier ip #!key line-begin line-end)
   (js-lexer-init!)
   (read/rp *js-lexer* ip 'hopscript line-begin line-end (not line-begin) '() 0))
...
(define (c-fontifier ip #!key line-begin line-end)
   (c-lexer-init!)
   (read/rp *js-lexer* ip 'c line-begin line-end (not line-begin) '() 0))
