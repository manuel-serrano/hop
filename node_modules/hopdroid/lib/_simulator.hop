...
(module _simulator
   ...
   (cond-expand
	  ...
	  (android-load-plugin ::androidphone ::bstring)
...
(define (android-load-plugin p::androidphone name)
   ...
   (define (load-plugin-js)
      (let ((name (string-append (prefix name) ".js")))
	 (when (file-exists? name)
	    (with-access::androidphone p (%worker %this %module)
	       ((nodejs-require %worker %this %module "hopscript")
		(js-undefined)
		name)))))
   
   (define (load-plugin-hop)
      (let ((name (string-append (prefix name) ".hop")))
	 (when (file-exists? name)
...
;*---------------------------------------------------------------------*/
(define-method (phone-contact::pair-nil this::androidphone)
   simulated-contacts)
...
;*---------------------------------------------------------------------*/
(define-method (phone-call-start this::androidphone no . optional)
   #t)
...
(define (simulator-trigger-event this::JsWrapper event . vals)
   (with-access::JsWrapper this (data)
      (with-access::androidphone data (events)
...
(define-service (hopdroid/simulator)
   (if phone-simulated
	  ...
	  (with-access::androidphone data (%this %worker)
...
(define (emit evt val)
   (with-access::JsWrapper phone-simulated (data)
      (with-access::androidphone data (%this %worker listeners)
	 (js-worker-exec %worker "hopdroid/simulator" #t
	    (lambda ()
