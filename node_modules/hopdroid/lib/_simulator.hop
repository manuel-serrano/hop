...
(module _simulator
   ...
   (cond-expand
	  ...
	  (android-load-plugin ::androidphone ::bstring ::vector ::obj)
...
;*    android-load-plugin ...                                          */
;*---------------------------------------------------------------------*/
(define (android-load-plugin p::androidphone name data plugindir)
   ...
   (if (eq? plugindir (js-undefined))
       ...
       (let* ((dir (js-tostring plugindir (vector-ref data 1)))
	      (path (make-file-name dir
...
;*---------------------------------------------------------------------*/
(define-method (phone-contact::pair-nil this::androidphone #!optional filter)
   simulated-contacts)
...
(define simulated-contacts
   (list (instantiate::vcard
      ...
      (instantiate::vcard
	    (familyname "Henry")
	    (firstname "mitch")
	    (emails '("mitch@henry.org"))
	    (phones '((default "2222-222-22")))
	    (addresses '())
	    (notes '((android-id . "34") (color . "#932"))))))
