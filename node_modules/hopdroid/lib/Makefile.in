#*=====================================================================*/
#*    .../project/hop/hop/node_modules/hopdroid/lib/Makefile.in        */
#*    -------------------------------------------------------------    */
#*    Author      :  Manuel Serrano                                    */
#*    Creation    :  Fri Aug 28 10:00:50 2015                          */
#*    Last change :  Mon May 18 20:39:50 2020 (serrano)                */
#*    Copyright   :  2015-21 Manuel Serrano                            */
#*    -------------------------------------------------------------    */
#*    hopdroid precompilation                                          */
#*=====================================================================*/
do: build

#*---------------------------------------------------------------------*/
#*    Configuration                                                    */
#*---------------------------------------------------------------------*/
PACKAGE=hopdroid
include ../../Makefile.modules

ifeq ($(ARCH),android)
HOPEXTRALIBS=hopdroid
else
HOPEXTRALIBS=
endif

#*---------------------------------------------------------------------*/
#*    The entries                                                      */
#*---------------------------------------------------------------------*/
.PHONY: build install uninstall hopdroid _hopdroid

build: .afile o
	$(MAKE) o/hopdroid.$(SHAREDSUFFIX) \
            TARGETNAME=hopdroid \
            OBJECTS=o/hopdroid.o
	$(MAKE) o/_hopdroid.$(SHAREDSUFFIX) \
            TARGETNAME=_hopdroid \
            OBJECTS="o/_hopdroid.o o/_simulator.o"

install:
	$(MAKE) mkdir DIR=$(DESTDIR)$(HOPLIBDIR)/$(LIBSDIR)
	$(MAKE) install-module TARGETNAME=_hopdroid
	$(MAKE) install-module TARGETNAME=hopdroid
	$(RM) -rf $(DESTDIR)$(HOPLIBDIR)/$(HOPFILDIR)/node_modules/$(PACKAGE)/lib/o
	chmod -R $(MODDIR) $(DESTDIR)$(HOPLIBDIR)/$(LIBSDIR)

install-module:
	$(MAKE) install-shared-lib \
                INSTALL="$(INSTALL)" \
                BOOTLIBDIR=$(BUILDDIR)/$(DIR)/o \
                FILDIR=$(LIBSDIR) \
                LIBDIR=$(HOPLIBDIR) \
                LIB=$(TARGETNAME)
	chmod -R $(MODDIR) $(DESTDIR)$(HOPLIBDIR)/hop/$(HOPRELEASE)/$(DIR)
	cp -r ../../$(PACKAGE) $(DESTDIR)$(HOPLIBDIR)/$(HOPFILDIR)/node_modules
	touch $(DESTDIR)$(HOPLIBDIR)/$(LIBSDIR)/$(TARGETNAME).$(SHAREDSUFFIX)
	chmod $(MODEXE) $(DESTDIR)$(HOPLIBDIR)/$(LIBSDIR)/$(TARGETNAME).$(SHAREDSUFFIX)

uninstall:
	$(MAKE) uninstall-module TARGETNAME=_hopdroid
	$(MAKE) uninstall-module TARGETNAME=hopdroid

uninstall-module:
	$(MAKE) uninstall-shared-lib LIB=$(TARGETNAME).$(SHAREDSUFFIX)

#*---------------------------------------------------------------------*/
#*    lib                                                              */
#*---------------------------------------------------------------------*/
o:
	mkdir o

hopdroid: o
	$(MAKE) o/$(TARGETNAME).$(SHAREDSUFFIX)

_hopdroid: o
	$(MAKE) o/$(TARGETNAME).$(SHAREDSUFFIX)

o/$(TARGETNAME).$(SHAREDSUFFIX): $(OBJECTS)
	$(MAKE) shared-lib \
             LDINSTALLNAMEDIR=$(DESTDIR)$(HOPLIBDIR)/$(LIBSDIR) \
             BOOTLIBDIR=$(BUILDDIR)/$(DIR)/o \
             FORCELD=true \
             LIBDEST=$@ \
             SONAME=$(TARGETNAME).$(SHAREDSUFFIX) \
             LDOPTS="-L$(BUILDLIBDIR) -L$(BIGLOOLIBDIR) $(LDOPTS)" \
             HOPBIGLOOEXTRALIBS="$(HOPBIGLOOEXTRALIBS) bigloomail bigloophone" \
             BIGLOOEXTRAAPPLIBS="$(HOPEXTRALIBS)"

#*---------------------------------------------------------------------*/
#*    .afile                                                           */
#*---------------------------------------------------------------------*/
.afile: _hopdroid.hop
	(cd $(dir $@) && $(AFILE) -suffix hop $(notdir $^) -o $(notdir $@))

#*---------------------------------------------------------------------*/
#*    clean                                                            */
#*---------------------------------------------------------------------*/
clean:
	$(RM) -rf o

devclean:
	$(RM) -f .afile

distclean: devclean

#*---------------------------------------------------------------------*/
#*    Suffixes                                                         */
#*---------------------------------------------------------------------*/
.SUFFIXES:
.SUFFIXES: .scm .o .hop .js

#*---------------------------------------------------------------------*/
#*    explicit rules                                                   */
#*---------------------------------------------------------------------*/
o/_hopdroid.o: _hopdroid.hop
	@ $(call compile3,$(HOPC),$(HFLAGS),$(BCFLAGS) -dload-sym,$(BLFLAGS),-c,$^ -o $@)

o/_simulator.o: _simulator.hop
	@ $(call compile3,$(HOPC),$(HFLAGS),$(BCFLAGS),$(BLFLAGS),-c,$^ -o $@)

#*---------------------------------------------------------------------*/
#*    The implicit rules                                               */
#*---------------------------------------------------------------------*/
o/%.o: %.scm
	@ $(call compile2,$(BIGLOO),$(BCFLAGS),$(BLFLAGS),-c,$< -o $@)

o/%.o: %.hop
	@ $(call compile3,$(HOPC),$(HFLAGS),$(BCFLAGS),$(BLFLAGS),-c,$< -o $@)

%.scm: %.hop
	@ $(call compile3,$(HOPC),$(HFLAGS),$(BCFLAGS),$(BLFLAGS),-s,$< -o $@)

o/%.o: %.js
	@ $(call compile3,$(HOPC),$(HFLAGS),$(BCFLAGS) -dload-sym,$(BLFLAGS),$(BHOPCFLAGS) -c --js-no-module-main --js-module-name __nodejs_$* --js-module-path $* --js-node-modules-dir $(BUILDDIR)/node_modules,$< -o $@)

