...
(module _hopdroid
   ...
   (library hopscript hop hopwidget nodejs mail)
...
(define (hopscript %this this scope module)
   (&with!
	 ...
	 (js-raise-type-error %this (format "not a phone (~a)" (typeof this)) this))
		  ...
		  :value (js-make-function %this
			    (lambda (this)
				   ...
				   (cond-expand
				      ((library hopdroid)
				       (with-access::JsWrapper this (data)
					  (js-phone-contact (phone-contact data)
					     (get-vcard-cmap %this) %this)))
				      (else
				       (js-vector->jsarray '#() %this)))
				   (not-a-phone this)))
			    (js-function-arity 0 0)
			    (js-function-info :name "contact" :len 0))
		  :writable #f
		  :enumerable #t)
	       ;; the prototype
	       obj)))
...
;*    js-phone-contact ...                                             */
;*---------------------------------------------------------------------*/
(define (js-phone-contact contacts vcard-cmap %this)

   (define (vcard->jsobject vcard)
      (with-access::vcard vcard (familyname firstname emails phones addresses)
	 (js-object-literal-init!
	    (instantiateJsObject
	       ...
	       (elements (vector
			    ...
			    ;; emails
			    (js-vector->jsarray
			       (map! (lambda (o) (js-obj->jsobject o %this))
				  (list->vector emails))
			       %this)
			    ;; phones
			    (js-vector->jsarray
			    ...
			    ;; addresses
			    (js-vector->jsarray
			       (map! (lambda (o) (js-alist->jsobject o %this))
				  (list->vector addresses))
			       %this)))))))

   (tprint "CONTACTS=" contacts)
   (js-vector->jsarray
      (vector-map! vcard->jsobject (list->vector contacts))
      %this))
