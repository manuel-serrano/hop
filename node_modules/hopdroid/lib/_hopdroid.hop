	    ...
	    (let ((obj (js-new0 %this js-object)))
		  ...
		  :value (js-make-function %this
			    (lambda (this plugin)
			       (if (phone? this)
				   (with-access::JsWrapper this (data)
				      (android-load-plugin data
					 (js-tostring plugin %this)))
				   (not-a-phone this)))
			    (js-function-arity 2 0)
			    (js-function-info :name "loadPlugin" :len 2))
		  :writable #f
		  :enumerable #t)
	       ;; sendCommand
	       (js-bind! %this obj (& "sendCommand")
		  :value (js-make-function %this
			    (lambda (this plugin . args)
			       (if (phone? this)
				   (with-access::JsWrapper this (data)
				      (apply android-send-command data
					 plugin
					 (map (lambda (o)
		  ...
		  :value (js-make-function %this
			    (lambda (this plugin . args)
				   ...
				   (with-access::JsWrapper this (data)
				      (js-obj->jsobject
					 (apply android-send-command/result data
					    plugin
					    (map (lambda (o)
						    (js-jsobject->obj o %this))
					       args))
		  ...
		  (lambda (data)
		     (with-access::androidphone data (name) name)))
	       ;; sdk
		  ...
		  (lambda (data)
		     (with-access::androidphone data (sdk-version) sdk-version)))
	       ;; model
		  ...
		  :get (js-make-function %this
			    (lambda (this)
			       (with-access::JsWrapper this (data)
				  (with-access::androidphone data (configuration)
				     (js-plist->jsobject
					configuration %this))))
			    (js-function-arity 0 0)
