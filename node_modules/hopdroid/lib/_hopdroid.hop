...
(define (js-phone-contact-list::JsArray phone::phone projection selection
	   this::JsWrapper cmap %this::JsGlobalObject)
   (js-vector->jsarray
      (vector-map! (lambda (vcard)
		      (vcard->jsobject vcard this cmap %this))
	 (list->vector (phone-contact-list phone projection selection)))
      %this))

;*---------------------------------------------------------------------*/
;*    js-phone-contact ...                                             */
;*---------------------------------------------------------------------*/
(define (js-phone-contact phone::phone id this cmap %this::JsGlobalObject)
   (vcard->jsobject (phone-contact phone id) this cmap %this))
   
;*---------------------------------------------------------------------*/
;*    vcard->jsobject ...                                              */
;*---------------------------------------------------------------------*/
(define (vcard->jsobject vcard this::JsWrapper cmap %this)
   ...
   (with-access::vcard vcard (id familyname firstname emails phones addresses photo notes)
	 ...
	 (instantiateJsObject
	    ...
	    (elements (vector
			 ...
			 id
			 ;; photo
			 (js-obj->jsobject photo  %this)
			 ;; url
			 (get-note 'url notes)
			 ;; color
			 (get-note 'color notes)
			 ;; origin
			 this))))))
