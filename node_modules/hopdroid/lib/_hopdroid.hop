;*=====================================================================*/
;*    .../hop/3.5.x/node_modules/hopdroid/lib/_hopdroid.hop            */
;*    -------------------------------------------------------------    */
;*    Author      :  Manuel Serrano                                    */
;*    Creation    :  Sun Jul  6 06:15:53 2014                          */
;*    Last change :  Sun Jan 23 16:30:07 2022 (serrano)                */
...
(module _hopdroid
   ...
   (cond-expand ((not (library hopdroid)) (import (_simulator "_simulator.hop"))))
...
(define (get-androidphone app)
   (or (find (lambda (p)
	  ...
	  phones)
       (let ((np (cond-expand
		    ((library hopdroid) (instantiate::androidphone (app app)))
		    (else (instantiate::nophone (app app))))))
   ...
   (&with!
	 ...
	 (with-access::JsGlobalObject %this (js-object)
	    (instantiateJsWrapper
	       ...
	       (data (vector (js-current-worker) %this module
			#unspecified '())))))
	 ...
	 (js-bind! %this obj name
      ...
      (define (bind-phone-int-getter! obj name getter)
	 (js-bind! %this obj name
	    :get (js-make-function %this
		    (lambda (this)
		       (if (phone? this)
	 ...
	 (unless vcard-cmap
	    (set! vcard-cmap
	       (js-strings->cmap
		  '#("familyname" "firstname" "nickname"
		     "emails" "phones" "addresses"
		     "id" "photo" "url" "birthday"
		     "sound" "lang" "related"
		     "color" "thumbnail" "key" "notes"))))
	 vcard-cmap)
	 ...
	 (with-access::JsGlobalObject %this (js-object)
	    (let ((obj (js-new0 %this js-object)))
		  ...
		  :value (js-make-function %this
			    (lambda (this key)
				   ...
				   (with-access::JsWrapper this (obj data)
				      (with-access::androidphone obj (name app)
					 (js-string->jsstring
					    (format "__androidphone:~a@~a__"
					       app name))))
				   (not-a-phone this)))
		  ...
		  (lambda (obj)
	       ...
	       ;; sdkVersion
	       (bind-phone-int-getter! obj (& "sdkVersion")
		  (lambda (obj)
		  ...
		  :writable #f
		  :enumerable #t)
	       (js-bind! %this obj (& "startActivity")
		  :value (js-make-function %this
			    (lambda (this action uri . opts)
			       (if (phone? this)
				   (with-access::JsWrapper this (obj)
				      (js-plist->jsobject
					 (apply android-start-activity obj
					    (js-tostring action %this)
					    (js-tostring uri %this)
					    (map (lambda (j)
						    (js-jsobject->obj j %this))
					       opts))
					 %this))
				   (not-a-phone this)))
			    (js-function-arity 2 -1 'scheme)
			    (js-function-info :name "startActivity" :len 2))
		  :writable #f
		  :enumerable #t)
	       (js-bind! %this obj (& "resolveHopActivity")
		  :value (js-make-function %this
			    (lambda (this action)
			       (if (phone? this)
				   (with-access::JsWrapper this (data)
				      (let ((c (assoc (js-tostring action %this)
						  (vector-ref data 4))))
					 (when (pair? c)
					    (cdr c))))))
			    (js-function-arity 1 0)
			    (js-function-info :name "resolveHopActivity" :len 2))
		  :writable #f
		  :enumerable #t)
	       (js-bind! %this obj (& "bindHopActivity")
		  :value (js-make-function %this
			    (lambda (this obj)
			       (if (phone? this)
				   (let ((action (js-tostring (js-get obj (& "action") %this) %this)))
				      (with-access::JsWrapper this (data)
					 (let ((c (assoc action (vector-ref data 4))))
					    (if (js-array? c)
						(js-array-push c obj %this #f)
						(vector-set! data 4
						   (cons action (js-vector->jsarray (vector obj) %this)))))))
				   (not-a-phone this)))
			    (js-function-arity 2 0)
			    (js-function-info :name "bindHopActivity" :len 2))
		  :writable #f
		  :enumerable #t)
	       ;; the prototype
	       obj)))
	 ...
	 (with-access::JsWrapper this (obj)
	    ...
	    (when (isa? obj nophone)
	       ;; an emulated phone
	       (cond-expand
		  ((not (library hopdroid)) (set-simulated-phone! this))
		  (else #unspecified))))
	 this)
      ...
      ;; emulator
      (js-bind! %this exports (& "emulator")
	 :value (cond-expand ((library hopdroid) #f) (else #t))
	 :writable #f
	 :enumerable #t)
...
(define (vcard->jsobject vcard cmap %this)
   ...
   (with-access::vcard vcard (uid familyname firstname nickname
				emails phones addresses photo
				url birthday sound lang related
				notes x-thumbnail x-color key)
