/*=====================================================================*/
/*    serrano/prgm/project/hop/3.6.x/node_modules/ts/lib/ts.js         */
/*    -------------------------------------------------------------    */
/*    Author      :  Manuel Serrano                                    */
/*    Creation    :  Tue Mar 13 07:37:34 2018                          */
/*    Last change :  Thu Mar  9 10:44:40 2023 (serrano)                */
...
function getTypeScriptModulesDir() {
   if (tsModulesDir) {
	 ...
	 tsModulesDir = process.env.HOP_TSMODULESDIR;
      	 ...
      	 const subdir = path.join(tmp, "hopts");
	 const dir = path.join(subdir, process.env.USER || "hop");
...
exports[Symbol.compiler] = (file, options) => {
   if (!file.match(/ts$/)) {
      return { type: "filename", value: file };
   } else {
      ...
      if (!fs.existsSync(target) || modified(file, target) || options["no-cache"]) {
      	 ...
      	 const main = options?.["module-main"] ?? true;
      	 ...
      	 if (debug) {
      	    console.log(`ts.js: compiling "${cmd}"`);
      	 }
      	 res = system(cmd);
      } else if (debug) {
	 console.log(`ts.js: using pre-compiled ${target}`);
      }
