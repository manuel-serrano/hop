/*=====================================================================*/
/*    serrano/prgm/project/hop/3.6.x/node_modules/ts/comp/comp.js      */
/*    -------------------------------------------------------------    */
/*    Author      :  Manuel Serrano                                    */
/*    Creation    :  Mon Mar 22 15:56:44 2021                          */
/*    Last change :  Tue Dec  5 18:47:08 2023 (serrano)                */
...
import * as hopc from "../../hopc/lib/ast.js"
...
function J2SString(node, val, env, scope) {
   return new hopc.J2SString(J2SLoc(node, env), // loc
      ...
      val, // value
      null, //escape
      node.kind === ts.SyntaxKind.PrivateIdentifier); // private
}

/*---------------------------------------------------------------------*/
/*    J2SStringPrivate ...                                             */
/*---------------------------------------------------------------------*/
function J2SStringPrivate(node, val, env, scope) {
   return new hopc.J2SString(J2SLoc(node, env), // loc
      J2SSymbol("string"), // type
      null, // hint
      undefined, // range
...
function J2SFunThis(node, env, scope, thisp, body) {
   return new (thisp ? hopc.J2SFun : hopc.J2SArrow)(J2SLoc(node, env), // loc
      ...
      undefined, // rrange
      thisp ? J2SSymbol("this") : J2SSymbol("%%nothis"), // idthis
      false, // idgen
      ...
      false, // vararg
      node.name ? J2SIdent(node.name, env, scope) : genname("fun"), // name
      false, // generator
      true, // optimize
      thisp, // thisp
      false, // argumentsp
      node.parameters ? node.parameters.map(p => J2SParam(p, env, scope)) : [], // params
      3, // constrsize
      true, // src
      false, // method
      false, // ismethodof
...
function J2SFunGen(node, env, scope, thisp, body) {
   return new hopc.J2SFun(J2SLoc(node, env), // loc
      ...
      J2SSymbol("unknown"), // new-target
      false, // vararg
      node.name ? J2SSymbol(node.name.escapedText + "*") : genname("fun*"), // name
      true, // generator
      true, // optimize
      thisp, // thisp
      false, // argumentsp
...
function J2SClassElement(node, env, scope) {
   ...
   function classElement(node, env, scope) {
      switch (node.kind) {
	    ...
	    const met = asyncFun(node)
	       ? J2SAsync(node, env, "local", J2SDeclThis(node, env, scope), mbody)
	       : J2SFunThis(node, env, "local", J2SDeclThis(node, env, scope), mbody);
   ...
   const isStatic = node.modifiers ? node.modifiers.find(m => m.kind === ts.SyntaxKind.StaticKeyword) : undefined;
...
function J2SDeclClass(node, env, scope) {
   ...
   function isRecordByDecorator(node) {
      return node.decorators && node.decorators.find(e => e && e.expression && e.expression.escapedText === "sealed");
   }
...
/*    J2SDeclVar ...                                                   */
/*---------------------------------------------------------------------*/
function J2SDeclVar(isvar, node, env, scope) {
   ...
   } else if (node.initializer || !isvar) {
...
function J2SVarDeclList(node, env, scope) {
   // as of feb 2023 there is no way to distinguish let and var statement
   // appart checking the parsed text!
   const isvar = node.getText().match(/^var /);
   const decls = node.declarations.map(d => J2SDeclVar(isvar, d, env, scope));
...
function J2SStmt(node, env, scope) {
   switch (node.kind) {
	 ...
	 return J2SDeclFun(node, env, scope);
      ...
      case ts.SyntaxKind.VariableDeclaration:
	 return J2SDeclVar(true, node, env, scope);
...
function main() {
   ...
   const prgm = new hopc.J2SProgram(`${src}:0`, // loc
      ...
      0, // rxcache-size
      0, // call-size
      src.replace(".ts", "") + "^" + createHash("sha256").update(src, "utf8").digest("hex").substring(0, 8), // name
      main == "true", // main
      false, // module
      null, // cnsts
      null, // decls
