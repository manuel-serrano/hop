...
/*    Last change :  Wed Dec 14 11:47:57 2022 (serrano)                */
...
function J2SFunBody(body, params, env, scope) {
   const inits = params.filter(n => n.initializer)
	 ...
	 const test = new hopc.J2SBinary(
	       ...
	       left, // left
	    right);
	 const then = new hopc.J2SStmtExpr(
	    loc, // loc
	    new hopc.J2SAssig(
	       loc, // loc
	 ...
	 const otherwise = new hopc.J2SNop(loc);
	 return new hopc.J2SIf(loc, test, then, otherwise);
      });

   if (inits) {
      inits.push(body);
      return new hopc.J2SBlock(
	 J2SLoc(body, env),
	 inits,
	 J2SLoc(body, env));
   } else {
