...
/*    Last change :  Wed Apr 28 11:34:04 2021 (serrano)                */
...
import * as _ts from "/usr/local/lib/node_modules/typescript/lib/typescript.js";
...
const options = { 
   ...
   typeRoots: [ "/usr/local/lib/hop/3.5.0/node_modules/@types" ]
};
...
function J2STemplate( node, env, scope ) {
   const head = J2SString( node.head, node.head.text, env, scope );
   const spans = [];
...
function J2SImport( node, env, scope ) {
   ...
   function J2SImportNamespace( node, env, scope ) {
	 ...
	 node.importClause.namedBindings.name;
      ...
      return new hopc.J2SImport( J2SLoc( node, env ), // loc
      	 node.moduleSpecifier.text, // path
	 [ new hopc.J2SImportNamespace( J2SLoc( node, env ), // loc
	      J2SIdent( name, env, scope ) ) ], // id
      	 false, // respath
      	 false, // mvar
      	 false, // ivar
         -1, // reindex
...
function J2STry( node, env, scope ) {
   function J2SCatch( node, env, scope ) {
 	 ...
 	 J2SDeclVar( node.variableDeclaration, env, scope ), // param
...
function J2SStmt( node, env, scope ) {
   switch( node.kind ) {
	 ...
	 return new hopc.J2SThrow( J2SLoc( node, env ), // loc
	    J2SExpr( node.expression, env, scope ) ); // expr
...
function main() {
   ...
   const main = process.argv[ 4 ] !== "false";
   const prog = ts.createProgram( [ src ], options );
   ...
   ts.forEachChild( file, n => {
      const m = J2SStmt( n, { source: src, typechecker: tc }, "global" );
