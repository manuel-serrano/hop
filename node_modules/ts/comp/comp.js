...
/*    Last change :  Fri Apr 16 19:00:45 2021 (serrano)                */
...
function J2SImport( node, env, scope ) {
   function J2SImportSpecified( node ) {
      const name = J2SIdent( node.name, env, scope );
      return J2SImportName( J2SLoc( node, env ), // loc
	 name, // name
	 nodem.PropertyName || name );
   }
   
   const names = 
      node.importClause.namedBindings.namedImports.elements.map( J2SImportSpecifier );
   
   return J2SImport( J2SLoc( node, env ), // loc
      J2SExpr( node.moduleSpecified ), // path
      names, // names
      false, // respath
      false, // mvar
      false, // ivar
