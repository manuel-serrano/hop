...
/*    Last change :  Thu Apr  1 18:05:59 2021 (serrano)                */
...
/*    J2SUndefined ...                                                 */
/*---------------------------------------------------------------------*/
function J2SUndefined( node, env, tc ) {
...
/*    J2SString ...                                                    */
/*---------------------------------------------------------------------*/
function J2SString( node, env, tc ) {
...
/*    J2SOperator ...                                                  */
/*---------------------------------------------------------------------*/
function J2SOperator( node, env, tc ) {
   switch( node.kind ) {
...
function J2SFun( node, env, tc ) {
   return new hopc.J2SFun( J2SLoc( node, env ), // loc
      ...
      false, // new-target
      false, // vararg
      node.name ? J2SIdent( node.name, env, tc ) : J2SSymbol( "fun" ), // name
      false, // generator
      true, // optimize
      false, // thisp
      false, // argumentsp
      node.parameters.map( p => J2SDecl( p, env, tc ) ), // params
      3, // constrsize
      true, // src
      false, // method
      false, // ismethodof
      J2SBlock( node.body, env, tc ) ); // body
}

/*---------------------------------------------------------------------*/
/*    J2SCall ...                                                      */
/*---------------------------------------------------------------------*/
function J2SCall( node, env, tc ) {
   return new hopc.J2SCall( J2SLoc( node, env ), // loc
      TCType( node, tc ), // type
      null, // hint
      undefined, // range
      -1, // profid
      J2SExpr( node.expression, env, tc ), // fun
      J2SSymbol( "direct" ), // protocol
      [ J2SUndefined( node, env, tc ) ], // thisarg
      node.arguments.map( n => J2SExpr( n, env, tc ) ) ); // args
}

/*---------------------------------------------------------------------*/
/*    J2SExpr ...                                                      */
/*---------------------------------------------------------------------*/
function J2SExpr( node, env, tc ) {
   switch( node.kind ) {
	 ...
	 return new hopc.J2SBinary( J2SLoc( node, env ), // loc
	    ...
	    null, // hint
	    undefined, // range
	    J2SOperator( node.operatorToken, env, tc ), // op
	    J2SExpr( node.left, env, tc ), // left
	    J2SExpr( node.right, env, tc ) ); // right

      case ts.SyntaxKind.CallExpression:
	 return J2SCall( node, env, tc );
	 ...
	 return new hopc.J2SAccess( J2SLoc( node, env ), // loc
	    ...
	    null, // hint
	    undefined, // range
	    J2SExpr( node.expression, env, tc ), // obj
	    J2SExpr( node.name, env, tc ) ); // field
	 
      case ts.SyntaxKind.FunctionExpression:
	 return J2SFun( node, env, tc );
	 ...
	 return new hopc.J2SAccess( J2SLoc( node, env ), // loc
	    ...
	    undefined, // range
	    J2SExpr( node.expression, env, tc ), // obj
	    J2SString( node.name, env, tc ) ); // field
	 ...
	 return new hopc.J2SParen( J2SLoc( node, env ), // loc
	    ...
	    null, // hint
	    undefined, // range
	    J2SExpr( node.expression, env, tc ) ); // expression
	 ...
	 return J2SUndefined( node, env, tc );
...
/*    J2SBlock ...                                                     */
/*---------------------------------------------------------------------*/
function J2SBlock( node, env, tc ) {
   return new hopc.J2SBlock( J2SLoc( node, env ), // loc
      node.statements.map( s => J2SStmt( s, env, tc ) ), // nodes
...
/*    J2SStmtExpr ...                                                  */
/*---------------------------------------------------------------------*/
function J2SStmtExpr( node, env, tc ) {
   return new hopc.J2SStmtExpr( J2SLoc( node, env ), // loc
      J2SExpr( node.expression, env, tc ) );
}

/*---------------------------------------------------------------------*/
/*    J2SVarDecls ...                                                  */
/*---------------------------------------------------------------------*/
function J2SVarDecls( node, env, tc ) {
   return new hopc.J2SStmtExpr( J2SLoc( node, env ), // loc
      node.declarationList.declarations.map( n => J2SDecl( n, env, tc ) ) ); // decls
}

/*---------------------------------------------------------------------*/
/*    J2SReturn ...                                                    */
/*---------------------------------------------------------------------*/
function J2SReturn( node, env, tc ) {
   return new hopc.J2SReturn( J2SLoc( node, env ), // loc
      false, // exit
      true, // tail
      undefined, // from
      J2SExpr( node.expression, env, tc ) ); // expr
}

/*---------------------------------------------------------------------*/
/*    J2SDecl ...                                                      */
/*---------------------------------------------------------------------*/
function J2SDecl( node, env, tc ) { 
   return new hopc.J2SDecl( J2SLoc( node, env ), // loc
      J2SIdent( node.name, env, tc ), // id
      false, // _scmid
      J2SKey(), // key
      true, // writable
      ...
      J2SType( node.type ), // utype
      J2SType( node.type ), // itype
      J2SType( node.type ), // vtype
      undefined, // irange
      undefined, // vrange
      null, // hint
      null ); // exports
...
/*    J2SDeclFun ...                                                   */
/*---------------------------------------------------------------------*/
function J2SDeclFun( node, env, tc ) {
   return new hopc.J2SDeclFun( J2SLoc( node, env ),  // loc
      J2SIdent( node.name, env, tc ), // name 
      false, // _scmid
      J2SKey(), // key
      false, // writable
...
/*    J2SStmt ...                                                      */
/*---------------------------------------------------------------------*/
function J2SStmt( node, env, tc ) {
   switch( node.kind ) {
      case ts.SyntaxKind.FunctionDeclaration: 
	 return J2SDeclFun( node, env, tc );
	 
      case ts.SyntaxKind.ReturnStatement:   
	 return J2SReturn( node, env, tc );
	 
      case ts.SyntaxKind.ExpressionStatement:   
	 return J2SStmtExpr( node, env, tc );
	 
      case ts.SyntaxKind.VariableStatement:
	 return J2SVarDecls( node, env, tc );

      case ts.SyntaxKind.EndOfFileToken:
	 return false;
	 
      default:
	 console.log( "J2SStmt unknown statement", ts.SyntaxKind[ node.kind ] );
	 return new hopc.J2SStmtExpr( J2SLoc( node, env ), 
	    J2SUndefined( node, env, tc ) );
   }
}
...
function main() {
   ...
   ts.forEachChild( file, n => {
      const m = J2SStmt( n, {source: files[ 0 ]}, tc );
