			...
			(let loop ((cards '()))
			   (let ((vc (read-vcard port)))
				  ...
				  (js-vector->jsarray
				     ...
				     %this)
				  (loop 
				     (cons (vcard->jsobject vc url vcard-cmap %this)
					cards))))))))
...
;*    vcard->jsobject ...                                              */
;*---------------------------------------------------------------------*/
(define (vcard->jsobject vcard origin cmap %this)
   ...
   (with-access::vcard vcard (id familyname firstname emails
				phones addresses photo notes)
