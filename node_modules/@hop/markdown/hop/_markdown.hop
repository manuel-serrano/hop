;*=====================================================================*/
;*    .../hop/hop/node_modules/@hop/markdown/hop/_markdown.hop         */
;*    -------------------------------------------------------------    */
;*    Author      :  Manuel Serrano                                    */
;*    Creation    :  Sun Jul  6 06:15:53 2014                          */
;*    Last change :  Tue Dec  3 10:03:25 2024 (serrano)                */
...
(define (hopscript %this this %scope %module)
   ...
   (js-export (include load loadDocument eval)
	 ...
	 (js-make-function %this
	    ...
	    (js-function-info :name "include" :len 1)))
	 ...
	 :value include :writable #f :enumerable #f)
...
(define (show v this)
   (cond
      ((or (number? v) (symbol? v) (keyword? v) (string? v))
       v)
      ((isa? v JsFunction)
       (typeof v))
      (else
       (js-jsobject->obj v this))))
...
(define (fontify-code lines lang::bstring this scope fontifier id)
   ...
   (let ((i (string-index lang #\:)))
      (if i
	  (let ((lang (js-string->jsstring (substring lang 0 i)))
		(lnum (string->number (substring lang (+fx i 1))))
		(f (js-get fontifier (js-ascii-name->jsstring "lineNumber") this)))
	     (if (integer? lnum)
		 (js-call3 this f (js-undefined) (fontify lines lang) lnum id)
		 (fontify lines lang)))
	  (fontify lines (js-string->jsstring lang)))))
...
(define *markdown-grammar*
   (regular-grammar ((punct (in "+=/-$~#%!'\""))
		     ...
		     (ident (: #\: (+ letter) (in " \t\n\r")))
		     (crlf (or "\n" "\r\n"))
		     srcpath charset this scope fontifier)

      (define (parse-string string srcpath charset this scope fontifier)
         (call-with-input-string string
	    (lambda (ip)
	       (markdown-parse ip srcpath charset this scope fontifier
		  (lambda l l)))))
	  ...
	  (cond
		 ...
		 (with-handler
		    (lambda (e)
		       (exception-notify e)
		       (token 'ERROR e 0))
		    (call-with-input-string (cadr m)
		       (lambda (port)
			  (let ((val (%js-eval port 'repl this (js-undefined) scope)))
			     (cond
				((isa? val JsStringLiteral)
				...
				((number? val)
				 (let ((str (number->string val)))
				    (rgc-buffer-insert-substring! (the-port) str 0
				       (string-length str))
				    (ignore)))
	...
	(with-handler
	   (lambda (e)
	      (display "HTML parsing error...\n" (current-error-port))
	      (exception-notify e)
	      (token 'ERROR e 0))
	   (let ((val (%js-eval (the-port) 'xml this (js-undefined) scope)))
	      (token 'HOP val 0))))
...
;*    markdown-parse ...                                               */
;*---------------------------------------------------------------------*/
(define (markdown-parse ip srcpath cset this scope fontifier ctor)
   ...
   (define (peek-token)
	  ...
	  (begin
	     (set! *peeked-tokens*
		(list (read/rp *markdown-grammar* ip srcpath
			 charset this scope fontifier)))
	     (car *peeked-tokens*))
	  (car *peeked-tokens*)))
   ...
   (define (parse-span-elements state::MDState tag end bra-as-text o-as-text)
      (cond
	 ((eq? (peek-token-type) end)
	  (consume-any!)
	  (values 'span
	     (apply tag :%context this
		state.attributes (reverse! state.elements))))
   ...
   (define (parse-span state::MDState tag end cleanup
   ...
   (define (section state::MDState)
      (let* ((tok (consume-any!))
	     ...
	     (title (if (string? (cdr sec))
			(html-string-encode (conv (cdr sec)))
			(cdr sec)))
	     (id #f))
	 (if tag
   ...
   (define (li state::MDState token)
      (let ((li (instantiate::MDState
		   ...
		   (ctor (lambda l
				...
				(<MARKDOWN:LI>
				   (apply <MARKDOWN:P> :%context this l))
				(apply <MARKDOWN:LI> l))))
   ...
   (define (pre state::MDState token indent)
	 ...
	 (values 'block
	    (<MARKDOWN:PRE> :%context this
	       (apply <MARKDOWN:CODE> :%context this lines)))))
   ...
   (define (inline-element? el)
      (if (not (isa? el xml-element))
	  #t
	  (with-access::xml-element el (tag)
	     (not (memq tag '(div footer ul pre))))))
   ...
   (define (span state::MDState)
	 ...
	 ((CODE2)
	  (let ((val (token-value (consume-any!))))
	     (parse-span state <MARKDOWN:CODE> 'CODE2 val
		:wrapper 'CODE2
		:bra-as-text #t
		:o-as-text #t))
	  #t)
	 ...
	 ((OBRA)
	  ...
	  (reference state
		...
		(<MARKDOWN:A> :%context this :href url :title title body))
	     (lambda l l))
	  #t)
	 ((IMAGE)
	  (let ((tok (consume-any!)))
		...
		(lambda (src title body)
		   (<MARKDOWN:IMG> :%context this
		      :src (src-align src (pwd) srcpath)
		      :title title :alt body))
		(lambda (els)
		   (apply string-append (filter string? els)))))
	  #t)
	 ((HOP)
	  (state-add! state (token-value (consume-any!)))
	  #t)
   ...
   (define (block state::MDState bob)
	 ...
	 ((eq? (peek-token-type) 'HOP)
	  (let ((el (token-value (consume-any!))))
	     (cond
		((isa? el JsArray)
		 (state-add! state (flatten el this))
		 (block state #f))
		((isa? el JsObject)
		 (values 'blocks (list (end-blocks state) #unspecified)))
		((inline-element? el)
		 (state-add! state el)
		 (block state #f))
		(else
...
(define (markdown-parse-file file charset this scope fontifier)
   ...
   (call-with-input-file file
      (lambda (ip)
	 (markdown-parse ip file charset this scope fontifier
	    (lambda l
	       (apply <MARKDOWN:P> :%context this l))))))
...
(define (markdown-parse-obj obj charset this scope fontifier)
   (if (eq? obj (js-undefined))
	  ...
	  (call-with-input-string str
	     (lambda (ip)
		(markdown-parse ip #f charset this scope fontifier
		   (lambda l
		      (apply <MARKDOWN:P> :%context this l))))))))
