...
);let strncpy = ((o, max) => { const loop$ = ((function loop(i) { if (i === max) return mm.substring(o, (o + i)); else { let c = mm.ref((i + o)); if (c === '\u0000') return mm.substring(o, (i + o)); else return loop((i + 1)); } })(0)); return loop$; });let readIfd = ((en, offset) => { return processExifDir$(en, mm, offset, 0, exif, 0, 0); }
);{ let en = tiffEndianess(), offset = get32u(en, mm, 4); return readIfd(en, offset); } }
)
const readRafSections = ((exif, mm, path) => { { let pos = mm.rindex, fmtv = mm.substring(pos, (pos + 4)), id = mm.substring((pos + 4), (pos + 12)); pos = mm.rindex;{ let buf = mm.substring(pos, (pos + 32)), i = buf.indexOf('\u0000'), model = buf.substring(0, i); { let obj1088 = exif; obj1088.make = "FUJIFILM";obj1088.uniqueCameraModel = model;obj1088.cameraSerialNumber = id;{ let version = mm.toString(4), unknown = mm.toString(20), jpegoff = mm.toString(4), metaoff = mm.toString(4), metalen = mm.toString(4), cfaoff = mm.toString(4), cfalen = mm.toString(4), unknownlen = mm.toString(12), someoff = mm.toString(4); mm.rindex = (mm.rindex + 28);return readJpegSections(exif, mm, path); } } } } }
)
const readRw2Sections = ((exif, mm, path) => { let tiffEndianess = (() => { return mm.ref(0) === 'M'; }
);let strncpy = ((o, max) => { const loop$ = ((function loop(i) { if (i === max) return mm.substring(o, (o + i)); else { let c = mm.ref((i + o)); if (c === '\u0000') return mm.substring(o, (i + o)); else return loop((i + 1)); } })(0)); return loop$; });let readEntry = ((en, offset) => { { let tag = get16u(en, mm, (0 + offset)), typ = get16u(en, mm, (2 + offset)), cnt = get32u(en, mm, (4 + offset)), voff = get32u(en, mm, (8 + offset)); return console.error("tag=", tag, " 0x", fixnumtostring(tag, 16), " typ=", typ, " cnt=", cnt, " voff=", voff, " ", (typ === 2) ? (strncpy(voff, cnt)) : undefined); } }
...
const jpegExif = ((path) => { if (!(fs.existsSync(path))) return error_errno($errnoIoFileNotFoundError, "jpeg-exif", "Can't find file", path); else { let mm = new Mmap(path, 'write', false), exf = new exif({}); try { if ((mm.length > 0)) if (tiffp(mm)) readTiffSections(exf, mm, path); else if (rw2p(mm)) readTiffSections(exf, mm, path); else if (rafp(mm)) readRafSections(exf, mm, path); else { mm.rindex = 0;readJpegSections(exf, mm, path); }
 } finally { mm.close() }return exf; } }
)
const jpegExifCommentSet$ = ((path, comment) => { if (!(fs.existsSync(path))) return error_errno($errnoIoFileNotFoundError, "jpeg-exif-comment-set!", "Can't find file", path); else { let mm = new Mmap(path), exf = new exif({}), mtime = false; try { if ((mm.length > 0)) { readJpegSections(exf, mm, path);{ let obj1089 = exf; return (obj1089['$commentpos']) && ((((len, s) => { mm.windex = obj1089['$commentpos'];mmapPutString$(mm, "ASCII\u0000\u0000\u0000");mmapPutString$(mm, s);mmapPutString$(mm, "\u0000");mtime = true;return s; })(comment.length, ((len < obj1089['$commentlen'])) ? (comment) : (comment.substring(0, obj1089['$commentlen']))))); } }
...
const jpegExifOrientationSet$ = ((path, orientation) => { if (!(fs.existsSync(path))) return error_errno($errnoIoFileNotFoundError, "jpeg-exif-comment-set!", "Can't find file", path); else { let mm = new Mmap(path), exf = new exif({}), mtime = false; try { if ((mm.length > 0)) { readJpegSections(exf, mm, path);{ let obj1090 = exf; if (obj1090['$orientationpos']) { mm.windex = obj1090['$orientationpos'];switch (orientation) { case 'landscape': mmapPutString$(mm, ""); break; case 'portrait': mmapPutString$(mm, ""); break; case 'upsidedonw': mmapPutString$(mm, ""); break; case 'seascape': mmapPutString$(mm, ""); break; }mtime = true;return orientation; }
 } }
 } finally { mm.close();if (mtime) { let pr = openInputFile(path), pw = appendOutputFile(path); { let c = readChar(pr); setOutputPortPosition$(pw, 0);writeChar(c, pw); }closeInputPort(pr);closeOutputPort(pw); } }
 } }
)
