;*=====================================================================*/
;*    serrano/prgm/project/hop/hop/node_modules/exif/lib/_exif.hop     */
;*    -------------------------------------------------------------    */
;*    Author      :  Manuel Serrano                                    */
;*    Creation    :  Sun Jul  6 06:15:53 2014                          */
;*    Last change :  Wed Feb 22 16:38:00 2023 (serrano)                */
...
;*    *exif-ignored-fields* ...                                        */
;*---------------------------------------------------------------------*/
(define *exif-ignored-fields*
   '(thumbnailOffset thumbnailLength %orientationpos %commentpos %commentlen))
...
;*    exif-class-fields ...                                            */
;*---------------------------------------------------------------------*/
(define (exif-class-fields clazz)
   (list->vector
      (filter (lambda (f)
		 (not (memq (class-field-name f) *exif-ignored-fields*)))
   ...
   (vector-map (lambda (k)
		  (let ((name (symbol->string! (class-field-name k)))
			...
			(%get (lambda (obj owner propname %this)
				 (with-access::JsWrapper obj (obj data)
				    (let ((v (get obj)))
				       (match-case v
					  ...
					  ((and ?a (? string?))
					   (if (eq? propname (& "thumbnail"))
					       (js-8bits->jsstring a)
					       (js-string->jsstring a)))
					  ((and ?a (? date?))
					   (js-date->jsdate a %this))
					  (else
					   (js-obj->jsobject v %this)))))))
			(%set (lambda (obj v owner propname %this)
				 #unspecified)))))
      (exif-class-fields clazz)))
...
(define (make-class-cmap clazz %this)
   (js-names->cmap
      (vector-map (lambda (f)
		     (js-prop->name (symbol->string! (class-field-name f))))
	 (exif-class-fields clazz))
	 ...
	 (with-access::JsGlobalObject %this (js-new-target)
	    (with-access::JsWrapper this (obj)
		   ...
		   (js-raise-type-error %this 
		      "Exif must be used as a constructor" this)
		   (begin
