...
;*    Last change :  Wed Jul 29 11:40:59 2015 (serrano)                */
   ...
   (define (href)
	 ...
	 (case (peek-token-type)
		...
		((eq? (peek-token-type) 'CPAR)
		 #f)
		(else
		 (parse-token-error "Illegal token \"~a\"" (consume-any!))))))
	 (values url title)))
	 ...
	 (if link
		...
		(let loop ()
		   (case (peek-token-type)
			  ...
			  (href)
			  (consume-token! 'CPAR)
			  (tprint "url=" url " title=" title " link=" link)
			  (state-add! state
			     (ctor url title
			  ...
			  (cond
			     ...
			     ((string=? (token-value tok) ":")
			      (tprint "TODO"))
			     (else
			      (parse-token-error "Illegal token \"~a\""
				 tok)))))
		      ((OBRA)
		       (consume-any!)
		       (let ((ref (lref)))
