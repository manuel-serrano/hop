...
;*    Last change :  Sun Oct  7 09:42:55 2018 (serrano)                */
...
(define (markdown-parse ip cset this scope fontifier)
   ...
   (define (start-itemize-in-p state::MDState token)
      (let* ((ul (instantiate::MDState
	     ...
	     (el (li ul token))
	     (nstate 
	 (let* ((nstate (duplicate::MDState state))
		    (pstate (end-subblock statel))
		
	     (list (end-blocks state) el)
	     el)))
      (let ((ul (instantiate::MDState
		   (tag (car token))
		   (ctor (if (eq? (car token) 'UL) <MARKDOWN:UL> <MARKDOWN:OL>))
