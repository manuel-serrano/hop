...
;*    Last change :  Thu Aug 27 08:40:51 2015 (serrano)                */
...
(define *tab-code-block-grammar*
   (regular-grammar ((line (or (* (out "\n,$"))
	  ...
	  (set! lines
	     (cons (%js-eval (the-port) 'repl ctx (js-undefined) ctx)
		lines)))
       (rgc-context 'hop)
       (ignore))
      (else
...
;*    fontify-code ...                                                 */
;*---------------------------------------------------------------------*/
(define (fontify-code lines lang ctx fontifier id)
   
   (define (fontify lines lang)
      (let ((f (when (isa? fontifier JsObject)
		  (js-get fontifier lang ctx))))
	 (if (isa? f JsFunction)
   ...
   (let ((i (string-index lang #\:)))
	  ...
	  (let ((lang (string->symbol (substring lang 0 i)))
		...
		(f (js-get fontifier 'lineNumber ctx)))
	     (if (integer? lnum)
		 (js-call3 ctx f (js-undefined) (fontify lines lang) lnum id)
		 (fontify lines (string->symbol lang))))
	  (fontify lines (string->symbol lang)))))
   ...
   (regular-grammar ((line (+ (out "`\n,$")))
	 ...
	 (call-with-input-string str
	  ...
	  (lambda (e)
	     (exception-notify e)
	     (set! lines (cons (<SPAN> :hssclass "hop-eval-error" e))))
	  (let ((val (%js-eval (the-port) 'repl ctx (js-undefined) ctx)))
	     (cond
		((string? val)
		 (set! lines (include-string val lines)))
		((isa? val JsStringLiteral)
		 (set! lines (include-string (js-tostring val ctx) lines)))
		(else
		 (cons val lines)))
	     (ignore))))
      (else
...
;*    markdown-parse ...                                               */
;*---------------------------------------------------------------------*/
(define (markdown-parse ip charset ctx fontifier)
   ...
   (define (span state::MDState)
	 ...
	 ((O__)
	  (let ((val (token-value (consume-any!))))
	     (parse-span state <MARKDOWN:STRONG> 'E__ val))
	  #t)
	 ((O**)
	  (let ((val (token-value (consume-any!))))
	     (parse-span state <MARKDOWN:STRONG> 'E** val))
	  #t)
	 ((CODE)
	  (let ((val (token-value (consume-any!))))
...
(define (markdown-jsobj->hop jsobj #!key (charset (hop-locale)) %this)
   (let* ((str (cond
      ...
      (unwind-protect
	 (markdown-parse ip :charset charset :ctx %this)
	 (close-input-port ip))))
...
(define (markdown-string->hop string #!key (charset (hop-locale)) env)
   (let ((ip (open-input-string string)))
      (unwind-protect
	 (markdown-parse ip :charset charset :ctx env)
	 (close-input-port ip))))

;*---------------------------------------------------------------------*/
;*    markdown-parse-file ...                                          */
;*---------------------------------------------------------------------*/
(define (markdown-parse-file file charset ctx fontifier)
   (hop-load-afile (dirname file))
   (call-with-input-file file
      (lambda (ip)
	 (markdown-parse ip charset ctx fontifier))))

;*---------------------------------------------------------------------*/
;*    markdown-parse-obj ...                                           */
;*---------------------------------------------------------------------*/
(define (markdown-parse-obj obj charset ctx fontifier)
