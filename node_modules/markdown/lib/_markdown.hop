...
;*    Last change :  Wed May 27 19:16:09 2020 (serrano)                */
...
(define (hopscript %this this scope module)
   (&with!
	 ...
	 :value (js-make-function %this
		   ...
		   (js-function-info :name "toMarkdown" :len 0))
	 :writable #f
	 :enumerable #f)))
...
(define (markdown-parse ip cset this scope fontifier)
   ...
   (define (start-itemize-no-p state::MDState token)
      (let ((ul (instantiate::MDState
		   (tag (car token))
		   (ctor (lambda l
			    (apply (if (eq? (car token) 'UL) <MARKDOWN:UL> <MARKDOWN:OL>)
			       :%context this
			       l)))
		   (value (car (token-value token)))
		   (parent state))))
   ...
   (define (span state::MDState)
	 ...
	 ((IMAGE)
	  ...
	  (reference state
		...
		(<MARKDOWN:IMG> :%context this :src src :title title :alt body))
	     (lambda l (caar l)))
	  #t)
	 ((HOP)
   ...
   (define (blocks)
	 ...
	 (multiple-value-bind (retcode val)
	    (let ((state (instantiate::MDState
			    (tag 'P)
			    (ctor (lambda l (apply <MARKDOWN:P> :%context this l))))))
	       (block state #t))
	    (case retcode
	       ((eof)
