...
;*    Last change :  Thu Nov 16 08:51:40 2023 (serrano)                */
...
(cond-expand
   ((not (library sqlite))
    (define (sqlite-get db proc query)
       (error "sqlite-get" "not configured" query))
    (define (sqlite-close db)
       (error "sqlite-close" "not configured" db))
    (define (sqlite-config conf)
       (error "sqlite-configure" "not configured" conf))
    (define (sqlite-for-each db proc query)
       (error "sqlite-for-each" "not configured" query))
    (define (sqlite-run db query)
       (error "sqlite-run" "not configured" query))
    (define (sqlite-name-of-tables db)
       (error "sqlite-name-of-tables" "not configured" db))
    (define (sqlite-table-name-of-columns db)
       (error "sqlite-table-name-of-columnss" "not configured" db))
    (define (sqlite-table-number-of-rows db)
       (error "sqlite-table-number-of-rows" "not configured" db))
    (define (sqlite-last-insert-rowid db)
       (error "sqlite-last-insert-rowid" "not configured" db))))
...
(define (hopscript %this this %scope %module)
   ...
   (js-export (Database format verbose)
	 ...
	 (let ((db (js-database-sqlite this %this)))
      ...
      (define (sqlite->js val)
	 (cond
	    ((string? val) (js-string->jsstring val))
	    ((number? val) val)
	    ((eq? val #unspecified) (js-undefined))
	    (else val)))
	    ...
	    (lambda (keys vals)
		  ...
		  (lambda (%this)
			...
			(instantiateJsObject
			   ...
			   (elements (vector-map! sqlite->js vals)))))))
	    query))
...
;* 			(condition-variable-wait! sync-condv sync-mutex)) */
	 ...
	 (let ((o (instantiateJsObject
	    ...
	    (js-bind! %this o (js-string->name "lastId")
	       :get (js-make-function-strict-lazy %this js-lastid
		       (js-function-arity js-lastid)
		       (js-function-info :name "lastId" :len 1)
		       0))
	    o))
...
(define (async-schedule! this::JsWrapper name proc::procedure)
   (with-access::JsWrapper this (obj)
      (with-access::sqliteasync obj (%this queue condv mutex async thread)
	 (synchronize mutex
	    (if thread
		(begin
	 ...
	 ((and completion
	  ...
	  (cond
	     ((not (js-object? (vector-ref &arguments 1)))
	      (values (-fx &len 3)
		 (vector-ref &arguments (-fx &len 2))
		 (vector-ref &arguments (-fx &len 1))))
	     ((=fx &len 4)
	      (values 1
		 (vector-ref &arguments (-fx &len 2))
		 (vector-ref &arguments (-fx &len 1))))
		 ...
		 (vector-ref &arguments (-fx &len 1))))
	     (else
	      (error "sqlite" "illegal arguments (2)" &arguments))))
	 ((and (>fx &len 1)
	       (js-callable? (vector-ref &arguments (-fx &len 1))))
	  ;; [, param, ...] [, callback]
	  (cond
	     ((not (js-object? (vector-ref &arguments 1)))
	      (values (-fx &len 2) (vector-ref &arguments (-fx &len 1)) #f))
	     ((=fx &len 3)
	      (values 1 (vector-ref &arguments (-fx &len 1)) #f))
	     ((=fx &len 2)
	      (values 0 (vector-ref &arguments (-fx &len 1)) #f))
	     (else
	      (error "sqlite" "illegal arguments (1)" &arguments))))
	 (else
	  ;; [, param, ...]
	  (cond
	     ((not (js-object? (vector-ref &arguments 1)))
