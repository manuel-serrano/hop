...
;*    Last change :  Sat Sep 24 12:04:54 2022 (serrano)                */
...
;*    constants                                                        */
;*---------------------------------------------------------------------*/
(defvar hopjs-parse-indent 3)
(defvar hopjs-parse-error-indent 1)
(defvar hopjs-parse-function-indent 1)
(defvar hopjs-parse-xml-indent 2)
...
(defvar hopjs-parse-initial-context
  (vector
   ...
   '())) ;; extra collapsing symbols

;*---------------------------------------------------------------------*/
;*    hopjs-parse-context-copy ...                                     */
;*---------------------------------------------------------------------*/
(defun hopjs-parse-context-copy (ctx)
  (vector (aref ctx 0) (aref ctx 1) (aref ctx 2) (aref ctx 3)))
...
(defsubst hopjs-parse-token-lcollapsingp (ctx tok)
  (let ((typ (hopjs-parse-token-type tok)))
    (or (memq typ '(export const let var return))
	(and (eq typ 'ident)
	     (consp (aref ctx 3))
	     (progn
	       (message "[%s]/[%s] -> %s %s %s" (hopjs-parse-token-string tok)
			(aref ctx 3)
			(member (hopjs-parse-token-string tok) (aref ctx 3))
			(stringp (hopjs-parse-token-string tok))
			(stringp (car (aref ctx 3))))
	       t)
	     (member (hopjs-parse-token-string tok) (aref ctx 3))))))
  ...
  (with-debug
   ...
   (let ((ftok (hopjs-parse-pop-token)))
	...
	(let ((itok (hopjs-parse-pop-token)))
	  (orn (hopjs-parse-args ctx ftok hopjs-parse-indent nil)
	       (hopjs-parse-return-type ctx ftok hopjs-parse-indent)
   ...
   (let ((ftok (hopjs-parse-pop-token))
	 ...
	 ((extends)
	  ...
	  (if (eq (hopjs-parse-peek-token-type) 'eop)
	      (setq res (hopjs-parse-token-column otok (+ indent hopjs-parse-indent)))
   ...
   (let ((etok (hopjs-parse-pop-token)))
	...
	(if (string= (hopjs-parse-token-string (hopjs-parse-peek-token)) "*")
	    (progn
	      ...
	      (case (hopjs-parse-peek-token-type)
		((eop)
		 (hopjs-parse-token-column otok (+ indent hopjs-parse-indent)))
		((from)
		 (hopjs-parse-pop-token)
	  ...
	  -2))
       ((lbrace)
	(orn (hopjs-parse-export-vars ctx etok hopjs-parse-indent)
	     (case (hopjs-parse-peek-token-type)
	       ((eop)
		(hopjs-parse-token-column otok (+ indent hopjs-parse-indent)))
	       ((semicolon)
	   ...
	   (let ((ntok (hopjs-parse-peek-token)))
		...
		(case (hopjs-parse-peek-token-type)
		  ...
		  ((as)
		   ...
		   (case (hopjs-parse-peek-token-type)
		     ((eop)
		      (setq res (hopjs-parse-token-column tok hopjs-parse-indent)))
		     ((ident)
		      (hopjs-parse-pop-token)
   ...
   (let ((etok (hopjs-parse-pop-token)))
	...
	(if (string= (hopjs-parse-token-string (hopjs-parse-peek-token)) "*")
	    (or (hopjs-parse-sequence
		 ctx otok (+ indent hopjs-parse-indent)
	...
	(hopjs-parse-pop-token)
	(hopjs-parse-stmt ctx etok hopjs-parse-indent))
       (t
	-200)))))
  ...
  (with-debug
   ...
   (let ((btok (hopjs-parse-pop-token)))
	...
	(hopjs-parse-token-column otok indent))
       ((lbrace)
	(if (eq (hopjs-parse-peek-token-type) 'eop)
	    (let ((collapse (and (hopjs-parse-token-lcollapsingp ctx otok)
				 (or t (hopjs-parse-token-in-linep otok btok)))))
	      (if collapse
		  (setq res (hopjs-parse-token-column otok indent))
		  (setq res (hopjs-parse-token-column otok (+ indent hopjs-parse-indent)))))
	  (let* ((res nil)
		 (collapse (and (hopjs-parse-token-lcollapsingp ctx otok)
				(or t (hopjs-parse-token-in-linep otok btok))))
		 (pshift (if collapse 0 hopjs-parse-indent))
		 (nshift (if collapse hopjs-parse-indent 0)))
	    (while (not res)
	      (let ((tok (hopjs-parse-peek-token)))
		(hopjs-debug
...
(defun hopjs-parse-if (ctx otok indent)
  (with-debug
   ...
   (let ((itok (hopjs-parse-pop-token)))
     (if (eq (hopjs-parse-peek-token-type) 'eop)
	 (hopjs-parse-token-column otok indent)
       (orn (hopjs-parse-paren-expr ctx itok hopjs-parse-indent)
	    (let ((indent (if (eq (hopjs-parse-peek-token-type) 'lbrace)
			      indent
			    (+ indent hopjs-parse-indent))))
...
(defun hopjs-parse-while (ctx otok indent)
  (with-debug
   ...
   (let ((wtok (hopjs-parse-pop-token)))
     (if (eq (hopjs-parse-peek-token-type) 'eop)
	 (hopjs-parse-token-column otok indent)
       (orn (hopjs-parse-paren-expr ctx wtok hopjs-parse-indent)
	    (let ((indent (if (eq (hopjs-parse-peek-token-type) 'lbrace)
			      indent
			    (+ indent hopjs-parse-indent))))
	      (hopjs-parse-stmt ctx otok indent)))))))
...
(defun hopjs-parse-decls (ctx otok indent)
  (with-debug
   ...
   (let ((dtok (hopjs-parse-pop-token)))
     ...
     (if (eq (hopjs-parse-peek-token-type) 'eop)
	 (hopjs-parse-token-column otok indent)
       (orn (hopjs-parse-vars ctx otok indent)
	    dtok)))))
;*        (orn (if otok                                                */
;* 		(hopjs-parse-vars ctx otok indent)                     */
;* 	      (hopjs-parse-vars ctx dtok hopjs-parse-indent))          */
;* 	    dtok)))))                                                  */
...
(defun hopjs-parse-var (ctx otok indent)
  (with-debug
   ...
   (case (hopjs-parse-peek-token-type)
	...
	(when (eq (hopjs-parse-peek-token-type) 'colon)
	  (let ((ty (hopjs-parse-type ctx otok l)))
	    (when (numberp ty)
	      (setq res ty))))
	(orn res
	     (case (hopjs-parse-peek-token-type)
		...
		(let ((etok (hopjs-parse-pop-token)))
		  ...
		  (if (eq (hopjs-parse-peek-token-type) 'eop)
		      (hopjs-parse-token-column otok hopjs-parse-indent)
		    (let ((e (hopjs-parse-assig-expr ctx otok hopjs-parse-indent)))
		      (orn e
			   (if (eq (hopjs-parse-peek-token-type) 'eop)
			       (hopjs-parse-token-column otok hopjs-parse-indent)
			     tok))))))
  ...
  (with-debug
   ...
   (let ((ftok (hopjs-parse-pop-token)))
	...
	(let ((tok (hopjs-parse-pop-token))
	  ...
	  (orn (hopjs-parse-stmt fctx ftok hopjs-parse-indent)
	       (if (memq (hopjs-parse-peek-token-type) '(in of))
		   (progn
		     (hopjs-debug 0 "hopjs-parse-for.in ntok=%s"
				  (hopjs-parse-peek-token))
		     (hopjs-parse-pop-token)
		     (orn (hopjs-parse-expr ctx tok hopjs-parse-indent)
			  (if (eq (hopjs-parse-peek-token-type) 'rparen)
				...
				(hopjs-parse-stmt ctx otok indent))
			    -1)))
		 (orn (hopjs-parse-stmt ctx ftok hopjs-parse-indent)
		      (hopjs-parse-expr ctx ftok hopjs-parse-indent)
		      (if (eq (hopjs-parse-peek-token-type) 'rparen)
			  (progn
			    (hopjs-parse-pop-token)
...
(defun hopjs-parse-throw (ctx otok indent)
  (with-debug
   ...
   (let ((ttok (hopjs-parse-pop-token)))
     (if (eq (hopjs-parse-peek-token-type) 'eop)
	 (hopjs-parse-token-column otok indent)
       (orn (hopjs-parse-expr ctx ttok hopjs-parse-indent)
	    ttok)))))
...
(defun hopjs-parse-switch (ctx otok indent)
  (with-debug
   ...
   (let ((stok (hopjs-parse-pop-token)))
     (if (eq (hopjs-parse-peek-token-type) 'eop)
	 (hopjs-parse-token-column otok indent)
       (orn (hopjs-parse-paren-expr ctx stok hopjs-parse-indent)
	    (hopjs-parse-stmt ctx otok indent)
	    stok)))))
       ...
       (orn (hopjs-parse-expr ctx otok indent)
	    (case (hopjs-parse-peek-token-type)
	      ((eop)
	       (hopjs-parse-token-column otok indent))
	      ((colon)
	       (hopjs-parse-case-block ctx ctok hopjs-parse-indent))
	      (t
	       -100)))))))
...
;*---------------------------------------------------------------------*/
;*    hopjs-parse-return-type ...                                      */
;*---------------------------------------------------------------------*/
(defun hopjs-parse-return-type (ctx otok indent)
  (with-debug
   "hopjs-parse-return-type (%s) otok=%s indent=%s ntok=%s callp=%s" (point)
   otok indent (hopjs-parse-peek-token) callp
   (if (eq (hopjs-parse-peek-token-type) 'colon)
       (hopjs-parse-type ctx otok l)
     nil)))
...
(defun hopjs-parse-args (ctx otok indent callp)
  ...
  (let ((ltok (hopjs-parse-pop-token)))
    (case (hopjs-parse-token-type ltok)
      ...
      ((lparen)
       (let ((res nil)
	     (aindent (if (and (hopjs-parse-token-lcollapsingp ctx otok)
			       (hopjs-parse-token-in-linep otok ltok))
			 indent
			 (+ hopjs-parse-indent indent)))
	     (atok otok)
	     (armed callp))
	 (while (not res)
	   (let ((ntok (hopjs-parse-peek-token)))
		...
		(let ((e (if (and (hopjs-parse-token-rcollapsingp (hopjs-parse-peek-token))
				  (hopjs-parse-token-in-linep ltok (hopjs-parse-peek-token)))
			     (hopjs-parse-arg-expr ctx otok indent)
			   (hopjs-parse-arg-expr ctx atok aindent))))
...
(defun hopjs-parse-expr (ctx otok indent)
  (with-debug
   ...
   (point) otok indent (hopjs-parse-peek-token)
   (let ((a (or (hopjs-parse-plugin-expr ctx otok indent)
...
;*---------------------------------------------------------------------*/
;*    hopjs-parse-plugin-expr ...                                      */
;*---------------------------------------------------------------------*/
(defun hopjs-parse-plugin-expr (ctx otok indent)
  (with-debug
   "hopjs-parse-plugin-expr (%s) otok=%s indent=%s ntok=%s"
   (point) otok indent (hopjs-parse-peek-token)
   (let ((p (assoc (hopjs-parse-token-string (hopjs-parse-peek-token))
		   (aref ctx 0))))
     (when (consp p)
	  ...
	  (case (hopjs-parse-peek-token-type)
	    ...
	    ((=)
	     (let* ((etok (hopjs-parse-pop-token))
	       ...
	       (hopjs-debug 0 "hopjs-parse-assig-expr.ret etok=%s tok=%s"
			    etok ntok)
	       (if (eq (hopjs-parse-token-type tok) 'eop)
		   (hopjs-parse-token-column otok hopjs-parse-indent)
		 (let ((eindent (if (hopjs-parse-token-in-linep etok ntok)
				    0
				  hopjs-parse-indent)))
...
;*---------------------------------------------------------------------*/
;*    hopjs-parse-arg-expr ...                                         */
;*---------------------------------------------------------------------*/
(defun hopjs-parse-arg-expr (ctx otok indent)
  (with-debug
   "hopjs-parse-arg-expr (%s) otok=%s indent=%s ntok=%s" 
   (point) otok indent (hopjs-parse-peek-token)
   (let ((l (hopjs-parse-assig-expr ctx otok indent)))
     (if (eq (hopjs-parse-peek-token-type) 'colon)
	 (hopjs-parse-type ctx otok l)
       l))))
...
(defun hopjs-parse-access-or-call (ctx otok indent e)
  (with-debug
   ...
   (letn axs ()
	 ...
	 ((dot)
	  (let ((tok (hopjs-parse-pop-token)))
	    ...
	    (unless (or (hopjs-parse-token-in-linep otok e)
	      ...
	      (setq otok e)
	      (setq indent hopjs-parse-indent)
	 ...
	 ((lbracket)
	  (let* ((tok (hopjs-parse-pop-token))
		 (e (hopjs-parse-expr ctx tok hopjs-parse-indent)))
	  ...
	  (t
	   (let ((e (hopjs-parse-expr ctx tok 1)))
	     ...
	     (orn e
		  (let ((res '()))
		    (while (not res)
		      (hopjs-debug 0 "hopjs-parse-primary.while tok=%s"
				   (hopjs-parse-peek-token))
		      (case (hopjs-parse-peek-token-type)
			((eop)
			 (setq res (hopjs-parse-token-column e)))
			((rparen)
			 (hopjs-parse-pop-token)
			 (hopjs-debug 0 "hopjs-parse-primary.rparen tok=%s"
				      (hopjs-parse-peek-token))
			 (when (eq (hopjs-parse-peek-token-type) 'colon)
			   (let ((ty (hopjs-parse-type ctx otok l)))
			     (when (numberp ty)
			       (setq res ty))))
			 (hopjs-debug 0 "hopjs-parse-primary.rparen.type tok=%s"
				      (hopjs-parse-peek-token))
			 (unless res
			   (case (hopjs-parse-peek-token-type)
			     ((eop) (setq res tok))
			   
			     ((=>) (setq res (hopjs-parse-arrow ctx otok tok indent)))
			     (t (setq res tok)))))
			((colon)
			 (let ((ty (hopjs-parse-type ctx otok l)))
			   (when (numberp ty)
			     (setq res ty))))
			((comma)
			 (setq res (hopjs-parse-expr ctx tok 1)))
			(t (setq res -1001))))
		    res)))))))
     ((lbracket)
  ...
  (with-debug
   ...
   (let* ((atok (hopjs-parse-pop-token))
	 ...
	 ((eop)
	  (if collapse
	      (hopjs-parse-token-column otok indent)
	    (hopjs-parse-token-column otok (+ hopjs-parse-indent indent))))
	 ((lbrace)
	  (let ((collapse2 (hopjs-parse-token-in-linep atok (hopjs-parse-peek-token)))
		...
		(if (eq (hopjs-parse-peek-token-type) 'eop)
		    (hopjs-parse-block ctx otok hopjs-parse-indent)
		  (progn
		...
		(if (eq (hopjs-parse-peek-token-type) 'eop)
		  ...
		  (progn
		    (hopjs-parse-push-token tok)
		    (hopjs-parse-block ctx otok (+ hopjs-parse-indent indent)))))
	       (collapse3
		(if (eq (hopjs-parse-peek-token-type) 'eop)
		    (hopjs-parse-block ctx otok hopjs-parse-indent)
		  (progn
  ...
  (with-debug
   ...
   (let ((ltok (hopjs-parse-pop-token)))
	...
	(hopjs-parse-token-column otok indent))
       ((lbrace)
	(let* ((res nil)
	       (collapse (and (or (hopjs-parse-token-lcollapsingp ctx otok)
		   ...
		   (case (hopjs-parse-peek-token-type)
			...
			(if (eq (hopjs-parse-peek-token-type) 'eop)
			  ...
			  (let* ((eindent (if (and (hopjs-parse-token-rcollapsingp
				...
				(case (hopjs-parse-token-type ntok)
				  ((eop)
				   (setq res (hopjs-parse-token-column otok (+ hopjs-parse-indent indent)))
				   (setq res (hopjs-parse-token-column otok indent)))
				  ((comma)
				   (hopjs-parse-pop-token))
				  ((rbrace)
...
(defun hopjs-parse-ohtml (ctx otok indent)
  (with-debug
   ...
   (let ((xtok (hopjs-parse-pop-token)))
	...
	(hopjs-parse-token-column otok indent))
       ((ident cssident class)
	(orn (hopjs-parse-xml-attrs ctx xtok hopjs-parse-indent)
	 ...
	 (case (hopjs-parse-token-type tok)
	   ...
	   ((ident cssident class)
	    (let ((itok (hopjs-parse-xml-attr-ident)))
	      (case (hopjs-parse-peek-token-type)
		((eop)
		 (setq res (hopjs-parse-token-column otok hopjs-parse-indent)))
		((=)
		 (hopjs-parse-pop-token)
		 (let ((e (hopjs-parse-primary ctx itok hopjs-parse-indent)))
		   (if (numberp e)
		       (setq res e)
		     (case (hopjs-parse-peek-token-type)
		       ((eop)
...
;*    hopjs-parse-type ...                                             */
;*---------------------------------------------------------------------*/
(defun hopjs-parse-type (ctx otok indent)
  (with-debug
   "hopjs-parse-type (%s) otok=%s indent=%s ntok=%s" 
   (point) otok indent (hopjs-parse-peek-token)
   (let ((res nil)
	 (last -10001))
     (hopjs-parse-pop-token)
	 ...
	 ((eop)
	  ...
	  (setq res t))
	 ((ident)
	  (setq last (hopjs-parse-pop-token)))
	 ((lbracket)
	  (hopjs-parse-pop-token)
	  (if (eq (hopjs-parse-peek-token-type) 'rbracket)
	      (progn
		(setq last (hopjs-parse-pop-token))
		(setq res t))
	    (setq res -10002)))
