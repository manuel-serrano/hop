...
;*    Last change :  Sat Apr 26 10:57:19 2008 (serrano)                */
...
(define (replay reqs)
   ...
   (let loop ((num (hoprp-threads-num)))
      (when (>fx num 0)
	 (thread-join! (thread-start-joinable! (make-thread replay-thread)))
	 (loop (-fx num 1)))))
...
(define (replay-thread)
   (let loop ()
	 ...
	 (let ((req (car *reqs*))
	    ...
	    (with-access::request req (host port method path)
